...
async function openWorkDetails(rowKey){if(controlBusy)return;if(selectedControlRow!==rowKey){controlDetail=null;controlBody.replaceChildren();controlHeading.textContent=""}selectedControlRow=rowKey;controlPanel.hidden=false;controlPanel.scrollIntoView({block:"nearest"});await refreshWorkDetails()}
