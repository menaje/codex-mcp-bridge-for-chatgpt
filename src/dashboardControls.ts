...
} from "./dashboardStopConfirmation.js";
...
function appendWorkControl(body,row){
 if(row.controlKind!=="request"&&row.controlKind!=="execution")return;
 const host=node("div","work-control"),button=actionButton("",()=>void openWorkDetails(row.rowKey)),chevron=node("span","chevron");
 host.dataset.controlRow=row.rowKey;button.className="work-control-toggle";chevron.setAttribute("aria-hidden","true");button.append(chevron,node("span","",t[row.controlKind==="request"?"dashboard.control.requests":"dashboard.control.work"]));
