...
const ENGLISH = {
  ...
  "settings.sharedNotice": "These settings are shared by all conversations using this MacBook Air bridge, not stored per ChatGPT account. Operator security policy cannot be changed here.",
...
const REMAINDER: Record<Exclude<SupportedUiLocale, "en" | "ko">, Partial<UiTranslationBundle>> = {
  ja: {
    ...
    "settings.sharedNotice": "この設定は ChatGPT アカウント別ではなく、この MacBook Air ブリッジを使用するすべての会話で共有されます。管理者のセキュリティポリシーはここでは変更できません。",
  ...
  "zh-Hans": {
    ...
    "settings.sharedNotice": "这些设置不会按 ChatGPT 账户保存，而是由使用此 MacBook Air 桥接的所有对话共享。管理员安全策略无法在此更改。",
  ...
  "zh-Hant": {
    ...
    "settings.sharedNotice": "這些設定不是依 ChatGPT 帳戶儲存，而是由使用此 MacBook Air 橋接的所有對話共用。管理員安全政策無法在此變更。",
  ...
  es: {
    ...
    "settings.sharedNotice": "Esta configuración se comparte entre todas las conversaciones que usan este puente del MacBook Air; no se guarda por cuenta de ChatGPT. La política de seguridad del operador no puede cambiarse aquí.",
  ...
  fr: {
    ...
    "settings.sharedNotice": "Ces paramètres sont partagés par toutes les conversations utilisant ce pont MacBook Air ; ils ne sont pas enregistrés par compte ChatGPT. La politique de sécurité de l’opérateur ne peut pas être modifiée ici.",
  ...
  de: {
    ...
    "settings.sharedNotice": "Diese Einstellungen werden von allen Unterhaltungen gemeinsam genutzt, die diese MacBook-Air-Bridge verwenden; sie werden nicht pro ChatGPT-Konto gespeichert. Die Sicherheitsrichtlinie des Betreibers kann hier nicht geändert werden.",
  ...
  pt: {
    ...
    "settings.sharedNotice": "Estas configurações são compartilhadas por todas as conversas que usam esta ponte do MacBook Air; não são salvas por conta do ChatGPT. A política de segurança do operador não pode ser alterada aqui.",
