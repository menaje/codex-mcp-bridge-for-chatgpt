...
function render(){
 const draft=formId===question?.questionId?Array.from(form.querySelectorAll("input,select"),field=>field.value):[];formId=null;form.replaceChildren();message.classList.remove("error");title.textContent=question?.title||t["question.title"];document.title=title.textContent;clearTimeout(expiryTimer);
 if(!question?.status){message.textContent=t["common.loading"];size();return}
 if(Date.now()>=question.expiresAt){message.textContent=t["question.expired"];size();return}expiryTimer=setTimeout(render,Math.max(1,question.expiresAt-Date.now()));
