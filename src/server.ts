...
export const BRIDGE_MCP_INSTRUCTIONS = [
  ...
  "The bridge independently performs bounded safe recovery: fresh non-loading inspection, release of confirmed idle persistent connections after failed or stopped work, and precise retries of an already requested stop. Finished failures are retained history and need no mandatory user acknowledgement. GPT recovery judgment is routed only through the original task call or a live exact-Job wait descended from it. Carry waitContext.token as waitToken in the next bounded codex_status Job/input wait, replacing it with the returned token after every wait. Tokens are single-use, conversation/Job/original-request bound, transient, and expire after a short gap; never obtain one from another conversation, invent one, or use it after ending this GPT response. Replays, overview/card/audit reads and historical exact-Job reads do not issue recovery directives. An origin-wait-recovery receipt concerns only that original Job and its user's task: assess the failure, continue only within existing task authorization, and verify the result. It never authorizes stopping another Job, terminating a shared worker, changing permissions, treating a failure as success, or waking any later/arbitrary GPT execution. If the original wait has ended, only bridge-safe automatic maintenance continues; unresolved operational issues stay in the native menu and dashboard.",
  ...
  "New work does not create automatic or full-history Activity cards. Do not call retired Activity presenters. The user opens codex_dashboard for the status card, codex_settings for configuration, or receives a codex_ask_user card when their opinion is needed. Background work continues independently of every card. Keep the current GPT response active through bounded codex_status input waits, then retrieve each terminal Job's retained result with an exact query {kind:'job',id}. A completed foreground task returns its bounded model-authoritative answer. Overview, Activity, thread and page queries never contain Job answer bodies. Tool content is compatibility-only and may be absent from the ChatGPT transcript. Distinguish delivered, omitted and unavailable results from Job terminality, and never start another codex_task merely to reconstruct a retained answer. Use codex_status without query for the scoped overview, or one query kind for authoritative detail, input, result, wait or page. Use codex_cancel with a unique requestId, target {kind:'job'|'activity',id}, exact authoritative expectedVersion and a short factual user-facing reason for explicit stop intent. Dashboard work details provide user-operated approval, stop and idle background-process controls with separate private proofs; they do not expand GPT's conversation scope. Observation abort, HTTP detach and widget unmount never cancel work. Use codex_agent for reversible archive, restore or rename. Operator recovery tools require explicit operator enablement. Interruption never rolls back filesystem changes. Retained old cards may continue their existing handoff during migration; their presence is not required for new work. Native Codex app notifications remain independent. A host acknowledgment does not prove a new GPT execution, and an already ended GPT response may require the user to resume it."
].join(" ");
