...
export const BRIDGE_MCP_INSTRUCTIONS = [
  ...
  "Call codex_task directly rather than through programmatic tool calling or an exec wrapper so ChatGPT preserves its native Activity UI. The bridge attaches and suppresses that UI from the saved visibility setting; never call codex_activity after codex_task. Use codex_activity only when the user explicitly asks to open or reopen the Activity view. A mounted card owns subsequent watches for its Activity generation. Use codex_status for authoritative detail or final job results. Use codex_cancel only to interrupt an active job, and codex_agent for reversible archive, restore, rename, detach, or exact background-process termination. Interruption and process termination never roll back filesystem changes."
].join(" ");
