...
export const BRIDGE_MCP_INSTRUCTIONS = [
  ...
  "Treat exact status waits as bounded reads whose timeout or host abort never cancels Codex work. A terminal wait wakes only for terminal lifecycle state. When the originating live Dashboard is mounted, it already watches completion; do not maintain a parallel terminal wait solely to duplicate that delivery, but continue to use bounded input waits and exact manual reads when needed.",
