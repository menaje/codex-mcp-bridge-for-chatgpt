...
export const SETTINGS_CARD_URI = `ui://${PRODUCT_INFO.runtimeName}/settings-v12.html`;
...
<body>
      ...
      <div class="grid">
        ...
        <section class="wide policy-panel" id="fixed-policy-panel">
          <div class="grid">
            ...
            <label><span data-i18n="settings.effort"></span><select id="policy-effort" required></select><span class="hint" data-i18n="settings.effortHint"></span></label>
  ...
  <script>
    ...
    function renderFixedEfforts(preferred) { const modelId=elements.model.value;const efforts=[...new Set(availableSelections().filter((selection)=>selection.model===modelId).map((selection)=>selection.reasoningEffort))];if(preferred&&preferred.model===modelId&&!efforts.includes(preferred.reasoningEffort))efforts.push(preferred.reasoningEffort);elements.effort.replaceChildren();for(const effort of efforts)elements.effort.appendChild(option(effort,effort));const wanted=preferred&&preferred.model===modelId?preferred.reasoningEffort:"";elements.effort.value=efforts.includes(wanted)?wanted:efforts[0]||"";renderFixedTiers(preferred); }
    ...
    elements.effort.addEventListener("change",()=>{modelPolicyDirty=true;renderFixedTiers(null);});
