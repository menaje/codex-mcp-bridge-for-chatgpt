...
${SETTINGS_PROJECT_ID_HELPERS}
    ...
    function projectErrorMessage(value) { if(value.includes("PROJECT_DUPLICATE_PATH"))return t["settings.projectDuplicatePath"];if(value.includes("PROJECT_LABEL_INVALID"))return t["settings.projectInvalidLabel"];if(value.includes("PROJECT_CWD_INVALID")||value.includes("PROJECT_CWD_NOT_ALLOWED"))return t["settings.projectInvalidCwd"];if(value.includes("PROJECT_DEFAULT_NOT_FOUND"))return t["settings.projectDefaultMissing"];if(value.includes("PROJECT_UNAVAILABLE")||value.includes("DEFAULT_CWD_NOT_ALLOWED"))return t["settings.projectUnavailableSave"];return t["settings.projectError"]; }
    ...
    function updateProjectEmptyState() { const count=projectRows().length;elements.noProjects.hidden=count>0;elements.defaultProject.disabled=count===0;elements.addProject.textContent=count===0?t["settings.addFirstProject"]:t["settings.addProject"];elements.addProject.disabled=count>=100;elements.addProject.title=count>=100?t["settings.projectLimit"]:""; }
    function projectRowKey(row) { return row.dataset.draftKey||row.dataset.projectId||""; }
    function projectOptionLabel(row) { const label=row.querySelector(".project-label-input").value.trim()||t["settings.projectNew"],cwd=row.querySelector(".project-cwd-input").value.trim();return label+(cwd?" · "+cwd:"")+(row.dataset.available==="false"?" ("+t["settings.projectUnavailable"]+")":""); }
    function renderDefaultProjectOptions(preferred) { const rows=projectRows();let wanted=preferred===undefined?elements.defaultProject.value:String(preferred||"");if(!wanted&&rows.length===1)wanted=projectRowKey(rows[0]);elements.defaultProject.replaceChildren(option("",t["settings.defaultProjectNone"]));for(const row of rows){const key=projectRowKey(row);if(key)elements.defaultProject.appendChild(option(key,projectOptionLabel(row)));}elements.defaultProject.value=[...elements.defaultProject.options].some((entry)=>entry.value===wanted)?wanted:""; }
    ...
    function appendProjectRow(project,persisted=false,available=true) {
      ...
      row.className="project-row"+(available?"":" unavailable");row.dataset.persisted=String(persisted);row.dataset.available=String(available);if(project.id)row.dataset.projectId=project.id;else row.dataset.draftKey="draft:"+nextProjectDraftId++;
      ...
      for(const input of[label,cwd])input.addEventListener("input",()=>{input.setCustomValidity("");clearProjectError();renderDefaultProjectOptions();});
    ...
    function buildProjectSettings() {
      ...
      const selectedDefaultKey=elements.defaultProject.value,selectedDefault=records.find((record)=>record.key===selectedDefaultKey),projects=records.map(({id,label,cwd})=>({id,label,cwd})),defaultProjectId=selectedDefaultKey?(selectedDefault&&selectedDefault.id||null):null;if(selectedDefaultKey&&!defaultProjectId){showProjectError(t["settings.projectDefaultMissing"]);return null;}return{projects,defaultProjectId};
    }
