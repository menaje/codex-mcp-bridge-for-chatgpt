...
export const SETTINGS_CARD_CONTRACT_GENERATION = 11;
...
<head>
  ...
  <style>
    :root { color-scheme: light dark; font-family: ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; --surface:color-mix(in srgb,Canvas 96%,CanvasText 4%); --muted:color-mix(in srgb,CanvasText 62%,transparent); --border:color-mix(in srgb,CanvasText 16%,transparent); --accent:#1777ff; --success:#16803a; --danger:#c34132; }
...
<body>
  <main class="card">
    ...
    <div class="catalog-status" id="catalog-status" role="status" aria-live="polite" data-state="invalid"><span id="catalog-status-label"></span><span class="catalog-status-source" id="catalog-status-source"></span></div>
  ...
  <script>
    ...
    function localizeProjectRows() { for(const row of projectRows()){row.querySelector(".project-label-title").textContent=t["settings.projectLabel"];row.querySelector(".project-cwd-title").textContent=t["settings.projectCwd"];const state=projectRowState(row),status=row.querySelector(".project-availability"),button=row.querySelector(".project-remove"),deleteButton=row.querySelector(".project-delete");status.textContent=state==="new"?t["settings.projectNew"]:state==="archive"?t["settings.projectArchivePending"]:state==="restore"?t["settings.projectRestorePending"]:state==="delete"?t["settings.projectDeletePending"]:state==="archived"?t["settings.projectArchived"]:row.dataset.available==="false"?t["settings.projectUnavailable"]:t["settings.projectAvailable"];button.hidden=state==="delete";button.textContent=state==="new"?t["settings.removeProject"]:state==="archived"||state==="archive"?t["settings.restoreProject"]:t["settings.archiveProject"];deleteButton.hidden=state!=="archived"&&state!=="delete";deleteButton.textContent=state==="delete"?t["settings.cancelDeleteProject"]:t["settings.deleteProject"];}updateProjectEmptyState(); }
    function appendProjectRow(project,persisted=false,available=true,archived=false) {
      const row=document.createElement("div"),label=document.createElement("input"),cwd=document.createElement("input"),actions=document.createElement("div"),status=document.createElement("span"),remove=document.createElement("button"),deleteButton=document.createElement("button");
      ...
      actions.append(status,remove,deleteButton);row.append(projectField("settings.projectLabel",label,"project-label-title"),projectField("settings.projectCwd",cwd,"project-cwd-title"),actions);elements.projectList.appendChild(row);
      ...
      remove.addEventListener("click",()=>{if(!persisted){row.remove();clearProjectError();updateProjectEmptyState();return;}const state=projectRowState(row);if(state==="active"){row.dataset.archive="true";}else if(state==="archive"){delete row.dataset.archive;}else if(state==="archived"){row.dataset.restore="true";}else if(state==="restore"){delete row.dataset.restore;}setDisabled();clearProjectError();localizeProjectRows();});
      deleteButton.addEventListener("click",()=>{const state=projectRowState(row);if(state==="delete"){delete row.dataset.delete;}else if(state==="archived"){if(!confirm(t["settings.deleteProjectConfirm"]))return;row.dataset.delete="true";}setDisabled();clearProjectError();localizeProjectRows();});
    ...
    function setError(error) { elements.status.classList.add("error");elements.status.textContent=localizedErrorMessage(error); }
    ...
    elements.retryModels.addEventListener("click",async()=>{setBusy(true,t["settings.refreshing"]);try{const next=unwrap(await callTool("codex_settings",{refreshModels:true}));render(next);setBusy(false,mutationStatus(next,t["settings.refreshed"]));}catch(error){setBusy(false);setError(error);}});
