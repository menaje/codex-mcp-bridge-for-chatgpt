...
export const SETTINGS_CARD_URI = `ui://${PRODUCT_INFO.runtimeName}/settings-v9.html`;
      ...
      contents: [
        {
          ...
          _meta: {
            ...
            "openai/widgetDescription":
              `Configure saved access, model, working-directory, interface-language, and concurrency defaults for ${PRODUCT_INFO.displayName}.`,
...
<body>
  ...
  <script>
    ...
    const elements = { form:byId("settings-form"),access:byId("access-strategy"),accessHint:byId("access-hint"),model:byId("default-model"),effort:byId("default-effort"),cwd:byId("default-cwd"),roots:byId("allowed-roots"),cwdHint:byId("cwd-hint"),language:byId("ui-language"),concurrency:byId("concurrency"),delivery:byId("completion-delivery"),save:byId("save"),refresh:byId("refresh"),reset:byId("reset"),status:byId("status"),revision:byId("revision"),fullWarning:byId("full-warning"),catalogWarning:byId("catalog-warning") };
    ...
    elements.language.addEventListener("change",()=>{localePreference=elements.language.value;setLocale(effectiveLocaleTag(),false);updateAccessNotice();});
    elements.form.addEventListener("submit",async(event)=>{event.preventDefault();if(!view||!elements.form.reportValidity())return;setBusy(true,t["settings.saving"]);try{const result=await callTool("codex_update_settings",{expectedRevision:view.settings.revision,accessStrategy:elements.access.value,defaultModel:elements.model.value||null,defaultReasoningEffort:elements.effort.value||null,defaultCwd:elements.cwd.value.trim()||null,uiLocalePreference:elements.language.value,maxConcurrentJobs:integerValue(elements.concurrency),completionDeliveryMode:elements.delivery.value});render(unwrap(result));setBusy(false,t["settings.saved"]);}catch(error){setBusy(false);setError(error);}});
