...
export const SETTINGS_CARD_CONTRACT_GENERATION = 18;
...
<body>
  ...
  <script>
    const BUNDLES = ${serializedUiTranslations(["common", "settings", "effort", "history"])};
    ...
    function renderExplicitPolicy() {
      ...
      for(const modelId of modelIds.filter((id)=>explicitSelectedModels.has(id))){
        ...
        for(const [effort,candidates] of groups){const primary=primarySelectionForEffort(modelId,effort,candidates);if(!primary)continue;const label=document.createElement("label"),checkbox=document.createElement("input"),effortAllowed=candidates.some((selection)=>availableKeys.has(selectionKey(selection)));label.className="checkline";checkbox.type="checkbox";checkbox.dataset.action="effort";checkbox.dataset.model=modelId;checkbox.dataset.effort=effort;checkbox.value=selectionKey(primary);checkbox.checked=exactSelectionsForEffort(modelId,effort).length>0;const blocked=isUltraDisabled(primary);checkbox.dataset.ultraDisabled=String(blocked);checkbox.disabled=blocked;label.append(checkbox,document.createTextNode((t["effort."+effort+".label"]||effort)+(blocked?" ("+t["settings.ultraDisabled"]+")":effortAllowed?"":" ("+t["settings.savedModel"]+")")));options.appendChild(label);}
    ...
    function renderHistorySettings(next){const policy=next.historyPolicy;elements.history.hidden=!policy||next.settings.historyRetentionDays===undefined;if(elements.history.hidden)return;elements.historyRetention.replaceChildren();for(const days of [7,30,90,0])elements.historyRetention.appendChild(option(String(days),days?t["history.days"].replace("{days}",String(days)):t["history.forever"]));elements.historyRetention.value=String(next.settings.historyRetentionDays);const values=[policy.retentionDays===0?t["history.unlimited"]:t["history.finite"].replace("{days}",String(policy.retentionDays)),t["history.notice"]];if(policy.lastCleanupAt)values.push(t["history.cleanup"].replace("{time}",new Intl.DateTimeFormat(localeTag,{dateStyle:"short",timeStyle:"short"}).format(new Date(policy.lastCleanupAt))).replace("{count}",String(policy.lastCleanupCount)));elements.historyPolicy.textContent=values.join("\n")}
