...
<body>
  ...
  <script>
    ...
    elements.form.addEventListener("submit",async(event)=>{event.preventDefault();if(!view)return;const projectSettings=buildProjectSettings();if(!projectSettings||!elements.form.reportValidity())return;setBusy(true,t["settings.saving"]);try{const args={expectedRevision:view.settings.revision,accessStrategy:elements.access.value,usePriorityServiceTier:elements.priority.checked,projects:projectSettings.projects,defaultProjectId:projectSettings.defaultProjectId,uiLocalePreference:elements.language.value,maxConcurrentJobs:integerValue(elements.concurrency),activityCardVisibility:elements.cardVisibility.value,completionHandoff:elements.handoff.value};if(modelPolicyDirty)args.modelPolicy=buildModelPolicy();const result=await callTool("codex_update_settings",args);render(unwrap(result));setBusy(false,t["settings.saved"]);}catch(error){await handleMutationError(error);}});
