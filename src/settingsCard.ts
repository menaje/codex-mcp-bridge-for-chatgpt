...
<body>
  <main class="card">
    ...
    <form id="settings-form">
      <div class="grid">
        ...
        <label><span data-i18n="settings.cardView"></span><select id="activity-card-view"><option value="agent-list" data-i18n="settings.cardView.agentList"></option><option value="activity-summary" data-i18n="settings.cardView.activitySummary"></option></select></label>
  ...
  <script>
    ...
    elements.form.addEventListener("submit",async(event)=>{event.preventDefault();if(!view||!elements.form.reportValidity())return;setBusy(true,t["settings.saving"]);try{const args={expectedRevision:view.settings.revision,accessStrategy:elements.access.value,defaultCwd:elements.cwd.value.trim()||null,uiLocalePreference:elements.language.value,maxConcurrentJobs:integerValue(elements.concurrency),activityCardVisibility:elements.cardVisibility.value,activityCardView:elements.cardView.value,completionHandoff:elements.handoff.value};if(modelPolicyDirty)args.modelPolicy=buildModelPolicy();const result=await callTool("codex_update_settings",args);render(unwrap(result));setBusy(false,t["settings.saved"]);}catch(error){await handleMutationError(error);}});
