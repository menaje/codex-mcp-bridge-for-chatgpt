...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; padding:12px; background:transparent; color:CanvasText; }
    ...
    .tier-options { display:grid; gap:6px; margin-top:6px; padding-left:4px; }
    ...
    @media (max-width:560px) { .grid,.explicit-layout{grid-template-columns:1fr}.wide{grid-column:auto}#status{text-align:left} }
  </style>
</head>
<body>
  <main class="card">
  ...
  <script>
    ...
    function primarySelectionForEffort(modelId,effort,candidates) { const availableKeys=new Set(availableSelections().map(selectionKey)),allowed=candidates.filter((selection)=>availableKeys.has(selectionKey(selection))),pool=allowed.length?allowed:candidates,model=modelFor(modelId),defaultTier=model&&model.defaultServiceTier;return pool.find((selection)=>defaultTier&&selection.serviceTier===defaultTier)||pool.find((selection)=>!selection.serviceTier)||pool[0]||null; }
    function seedExplicitModel(modelId) { if([...explicitSelectionMemory.values()].some((selection)=>selection.model===modelId))return;const available=modelSelections(modelId,availableSelections()),modelDefault=defaultSelectionForModel(modelId),seed=available.find((selection)=>modelDefault&&selectionKey(selection)===selectionKey(modelDefault))||available.find((selection)=>modelDefault&&selection.reasoningEffort===modelDefault.reasoningEffort&&!selection.serviceTier)||available[0];if(seed)explicitSelectionMemory.set(selectionKey(seed),seed); }
    ...
    function renderExplicitPolicy() {
      ...
      for(const modelId of modelIds.filter((id)=>explicitSelectedModels.has(id))){
        ...
        const extras=[...groups].flatMap(([effort,candidates])=>{const primary=primarySelectionForEffort(modelId,effort,candidates);return candidates.filter((selection)=>!primary||selectionKey(selection)!==selectionKey(primary));});
        if(extras.length){const details=document.createElement("details"),summary=document.createElement("summary"),tiers=document.createElement("div");summary.textContent=t["settings.additionalServiceTiers"];tiers.className="tier-options";for(const selection of extras){const label=document.createElement("label"),checkbox=document.createElement("input"),key=selectionKey(selection),missing=!availableKeys.has(key);label.className="checkline";checkbox.type="checkbox";checkbox.dataset.action="exact";checkbox.value=key;checkbox.checked=explicitSelectionMemory.has(key);label.append(checkbox,document.createTextNode((t["effort."+selection.reasoningEffort+".label"]||selection.reasoningEffort)+" · "+(selection.serviceTier||t["settings.serviceTier.default"])+(missing?" ("+t["settings.savedModel"]+")":"")));tiers.appendChild(label);}details.append(summary,tiers);card.appendChild(details);}
    ...
    function updatePolicyControls() { const fixed=elements.mode.value==="fixed",explicit=!fixed&&elements.allowedScope.value==="explicit";elements.fixedPanel.hidden=!fixed;elements.automaticPanel.hidden=fixed;elements.explicitPanel.hidden=!explicit;elements.model.disabled=!fixed;elements.effort.disabled=!fixed;elements.serviceTier.disabled=!fixed;elements.allowedScope.disabled=fixed;elements.preferred.disabled=fixed;if(explicit)ensureExplicitSelection();for(const checkbox of[...elements.allowedModels.querySelectorAll('input[type="checkbox"]'),...elements.effortGroups.querySelectorAll('input[type="checkbox"]')])checkbox.disabled=!explicit;renderPreferred(selectionFromKey(elements.preferred.value)); }
