...
<body>
  ...
  <script>
    ...
    elements.form.addEventListener("submit",async(event)=>{event.preventDefault();if(!view||!elements.form.reportValidity())return;setBusy(true,t["settings.saving"]);try{const args={expectedRevision:view.settings.revision,accessStrategy:elements.access.value,usePriorityServiceTier:elements.priority.checked,defaultCwd:elements.cwd.value.trim()||null,uiLocalePreference:elements.language.value,maxConcurrentJobs:integerValue(elements.concurrency),activityCardVisibility:elements.cardVisibility.value,completionHandoff:elements.handoff.value};if(modelPolicyDirty)args.modelPolicy=buildModelPolicy();const result=await callTool("codex_update_settings",args);render(unwrap(result));setBusy(false,t["settings.saved"]);}catch(error){await handleMutationError(error);}});
