...
export const SETTINGS_CARD_CONTRACT_GENERATION = 19;
...
<body>
  ...
  <script>
    const BUNDLES = ${serializedUiTranslations(["common", "settings", "effort", "history", "problem.historyNotice"])};
    ...
    function renderHistorySettings(next){const policy=next.historyPolicy;elements.history.hidden=!policy||next.settings.historyRetentionDays===undefined;if(elements.history.hidden)return;elements.historyRetention.replaceChildren();for(const days of [7,30,90,0])elements.historyRetention.appendChild(option(String(days),days?t["history.days"].replace("{days}",String(days)):t["history.forever"]));elements.historyRetention.value=String(next.settings.historyRetentionDays);const values=[policy.retentionDays===0?t["history.unlimited"]:t["history.finite"].replace("{days}",String(policy.retentionDays)),t[policy.reviewUntilRetention?"problem.historyNotice":"history.notice"]];if(policy.lastCleanupAt)values.push(t["history.cleanup"].replace("{time}",new Intl.DateTimeFormat(localeTag,{dateStyle:"short",timeStyle:"short"}).format(new Date(policy.lastCleanupAt))).replace("{count}",String(policy.lastCleanupCount)));elements.historyPolicy.textContent=values.join("\n")}
