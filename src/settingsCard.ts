...
export const SETTINGS_CARD_CONTRACT_GENERATION = 10;
...
<body>
  ...
  <script>
    ...
    function projectRowState(row) { return row.dataset.restore==="true"?"restore":row.dataset.archive==="true"?"archive":row.dataset.archived==="true"?"archived":row.dataset.persisted==="false"?"new":"active"; }
    function localizeProjectRows() { for(const row of projectRows()){row.querySelector(".project-label-title").textContent=t["settings.projectLabel"];row.querySelector(".project-cwd-title").textContent=t["settings.projectCwd"];const state=projectRowState(row),status=row.querySelector(".project-availability"),button=row.querySelector(".project-remove");status.textContent=state==="new"?t["settings.projectNew"]:state==="archive"?t["settings.projectArchivePending"]:state==="restore"?t["settings.projectRestorePending"]:state==="archived"?t["settings.projectArchived"]:row.dataset.available==="false"?t["settings.projectUnavailable"]:t["settings.projectAvailable"];button.textContent=state==="archived"||state==="archive"?t["settings.restoreProject"]:t["settings.archiveProject"];}updateProjectEmptyState(); }
    function appendProjectRow(project,persisted=false,available=true,archived=false) {
      ...
      const setDisabled=()=>{const disabled=projectRowState(row)==="archived"||projectRowState(row)==="archive";label.disabled=disabled;cwd.disabled=disabled;};setDisabled();
    ...
    function buildProjectOperations(projects) { const previous=new Map((view.settings.projects||[]).map((project)=>[project.id,project])),operations=[];for(const project of projects){const saved=project.id&&previous.get(project.id);if(!saved){operations.push({kind:"add",project:{name:project.name,cwd:project.cwd}});continue;}if(project.state==="archive"){operations.push({kind:"archive",projectId:project.id});continue;}if(project.state==="restore"){operations.push({kind:"restore",projectId:project.id,name:project.name,cwd:project.cwd});continue;}if(project.state==="archived")continue;if(project.name!==saved.name)operations.push({kind:"rename",projectId:project.id,name:project.name});if(project.cwd!==saved.cwd)operations.push({kind:"relocate",projectId:project.id,cwd:project.cwd});}return operations; }
