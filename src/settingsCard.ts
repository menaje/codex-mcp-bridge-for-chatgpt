...
export const SETTINGS_CARD_CONTRACT_GENERATION = 9;
...
<body>
  ...
  <script>
    ...
    elements.reset.addEventListener("click",async()=>{if(!view)return;setBusy(true,t["settings.resetting"]);try{render(unwrap(await callTool("codex_update_settings",{expectedSettingsRevision:view.settings.settingsRevision,operation:{kind:"reset"}})));setBusy(false,t["settings.resetDone"]);}catch(error){await handleMutationError(error);}});
