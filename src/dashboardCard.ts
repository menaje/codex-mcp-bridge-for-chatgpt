...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 23;
...
<body>
  <main class="card">
    ...
    <section id="work-details" hidden><header class="header"><h2 id="work-details-title"></h2><div><button id="work-details-refresh" type="button" data-i18n="common.refresh"></button><button id="work-details-close" type="button" data-i18n="activity.hideDetails"></button></div></header><p id="work-details-message" class="message" role="status"></p><div id="work-details-body"></div></section>
  ...
  <script>
    const BUNDLES=${serializedUiTranslations(["common", "usage", "cancellation", "dashboard", "activity.lastChanged", "activity.viewDetails", "activity.hideDetails", "activity.forceStop", "activity.forceConfirmTitle", "activity.forceConfirm", "activity.partialChanges", "activity.approve", "activity.approveSession", "activity.decline", "activity.answer", "activity.inputRequired", "activity.approval", "activity.optionalInput", "activity.openRequest", "activity.otherAnswer", "activity.yes", "activity.no", "activity.backgroundUnavailable", "question.gptHandles"])};
    ...
    function appendAgentBody(body,row,grouped=false,suppressIdleStatus=false,enclosingActivityTitle=null){const head=node("div","row-head"),title=node("div","row-title",row.agentName||t["dashboard.jobFallback"]),state=node("span","status "+tone(row.status),statusLabel(row.status)),turn=latestTurn(row),rowKey=historyKey(row);head.appendChild(title);if(!suppressIdleStatus||row.status!=="idle")head.appendChild(state);body.appendChild(head);if(!grouped&&turn)body.appendChild(node("div","turn-title",turnActivityTitle(turn)));appendRowContext(body,row,grouped?"agent":"row");const meta=rowMeta(row);if(meta)body.appendChild(node("div","meta",meta));if(turn){body.appendChild(node("div","time",timeMeta(turn,row.status)));appendExecution(body,turn.execution,false,true);if(!grouped||turn.cancellation&&turn.cancellation.targetKind!=="activity")appendCancellation(body,turn.cancellation,rowKey+"\u0000current\u0000"+String(turn.cancellation&&turn.cancellation.requestedAt||""))}else if(row.bucket!=="active")body.appendChild(node("div","time",[t["dashboard.time.durationUnknown"],relativeTime(row.updatedAt)].filter(Boolean).join(" · ")));if(shouldShowDashboardNextExecution(row.execution,turn&&turn.execution))appendExecution(body,row.execution,true);appendAgentHistory(body,row,turn,enclosingActivityTitle);const details=actionButton(t["activity.viewDetails"],()=>void openWorkDetails(row.rowKey));details.className="conversation-link";body.appendChild(details)}
...
${DASHBOARD_CONTROL_SCRIPT}
    ...
    async function selectScope(mode){if(!mounted||mode===selectedScope||mode==="conversation"&&!conversationAvailable)return;hydrationEpoch++;appendRequest=null;setBusy(false);closeWorkDetails();selectedScope=mode;view=null;activeRows=[];terminalRows=[];idleRows=[];terminalPagination=null;idlePagination=null;lastRenderedAt=0;statusIdleExpanded=false;expandedHistories.clear();expandedCancellations.clear();elements.activeList.replaceChildren();elements.terminalList.replaceChildren();elements.idleList.replaceChildren();elements.content.hidden=true;elements.updated.textContent="";syncScope();await reload(true)}
