...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 31;
...
${PROBLEM_REVIEW_MARKUP.trimStart()}
  ...
  <script>
    ...
    const initialDashboardOpen=initialMetadata&&initialMetadata[DASHBOARD_OPEN_METADATA_KEY],automaticDashboardOpen=initialDashboardOpen&&initialDashboardOpen.automatic===true&&initialDashboardOpen.scope==="conversation",presentationToken=automaticDashboardOpen&&typeof initialDashboardOpen.presentationToken==="string"?initialDashboardOpen.presentationToken:null;
    ...
    function dashboardPresentationArgs(){return presentationToken&&selectedScope==="conversation"?{presentationToken}:{}}
    function completionResult(value){const result=normalizeHostToolResult(value),candidate=result&&result.structuredContent||result;if(result&&result.isError)throw new Error(errorText(result));if(!candidate||candidate.kind!=="completion-delivery"||!Array.isArray(candidate.events))throw new Error(t["common.error"]);return candidate}
    function completionMessageOutcome(result){if(result&&typeof result==="object"){if(result.isError===true||result.error)return{accepted:false,uncertain:false};return{accepted:true,uncertain:false};}return{accepted:false,uncertain:true}}
    async function sendCompletionFollowUp(prompt){const ready=standardBridgeInitialized||await beginStandardBridge();if(ready){const result=await rpcRequest("ui/message",{role:"user",content:[{type:"text",text:prompt}]},10000,"COMPLETION_MESSAGE_TIMEOUT");return{attempted:true,...completionMessageOutcome(result)};}if(window.openai&&typeof window.openai.sendFollowUpMessage==="function"){const result=await withUiToolCallTimeout(()=>window.openai.sendFollowUpMessage({prompt}),10000,t["common.error"]);return{attempted:true,...completionMessageOutcome(result)};}return{attempted:false,accepted:false,uncertain:false};}
    function completionPrompt(events){return"Codex completion event(s) are ready in this conversation. Review the Dashboard before deciding whether any user-facing follow-up is needed. Event IDs: "+events.map(event=>event.eventId).sort().join(", ")+"."}
    function completionAction(action,outboxIds){return callTool("codex_ui_problem",{action,outboxIds,presentationToken,widgetInstanceId},false)}
    function scheduleCompletionDelivery(next){if(completionDeliveryTimer){clearTimeout(completionDeliveryTimer);completionDeliveryTimer=0;}const delivery=next&&next.completionDelivery;if(!mounted||!presentationToken||selectedScope!=="conversation"||!delivery||delivery.route!=="dashboard")return;const delay=completionDeliveryInFlight?1000:delivery.events.length?500:5000;completionDeliveryTimer=setTimeout(()=>{completionDeliveryTimer=0;if(!mounted||selectedScope!=="conversation"||completionDeliveryInFlight)return;if(view&&view.completionDelivery&&view.completionDelivery.events.length)void dispatchCompletionDelivery(view.completionDelivery);else void reload(true,false)},delay)}
    async function dispatchCompletionDelivery(delivery){if(completionDeliveryInFlight||!mounted||!presentationToken||selectedScope!=="conversation"||!Array.isArray(delivery.events)||delivery.events.length===0)return;completionDeliveryInFlight=true;let claimed=[],phase="before-send";try{const claim=completionResult(await completionAction("completion-claim",delivery.events.map(event=>event.outboxId)));claimed=claim.events||[];if(!claimed.length)return;phase="dispatching";const sent=await sendCompletionFollowUp(completionPrompt(claimed));if(!sent.attempted||!sent.accepted){await completionAction(sent.attempted&&sent.uncertain?"completion-uncertain":"completion-release",claimed.map(event=>event.outboxId));return;}phase="accepted";completionResult(await completionAction("completion-delivered",claimed.map(event=>event.outboxId)));}catch(error){if(claimed.length){try{await completionAction(phase==="before-send"?"completion-release":"completion-uncertain",claimed.map(event=>event.outboxId));}catch{}}showError(error)}finally{completionDeliveryInFlight=false;if(mounted)void reload(true,false)}}
...
${DASHBOARD_CONTROL_SCRIPT}
    ...
    function supersedeRequests(reason){hydrationEpoch++;appendRequest=null;enrichmentTarget=null;if(completionDeliveryTimer){clearTimeout(completionDeliveryTimer);completionDeliveryTimer=0;}setBusy(false);cancelPending(reason);if(!standardBridgeInitialized)standardBridgeAttempt=null}
