...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 32;
...
${PROBLEM_REVIEW_MARKUP.trimStart()}
  ...
  <script>
    ...
    let presentationInputRef=null,presentationResultRef=null,presentationLinked=false,presentationOutputReceived=false;
    function validPresentationRef(value){return typeof value==="string"&&/^[a-f0-9]{64}$/.test(value)}
    function presentationInput(value){const envelope=value&&typeof value==="object"&&!Array.isArray(value)?value:null,args=envelope&&envelope.arguments&&typeof envelope.arguments==="object"&&!Array.isArray(envelope.arguments)?envelope.arguments:envelope;return args&&args.scope==="conversation"&&typeof args.jobId==="string"&&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(args.jobId)&&validPresentationRef(args.presentationRef)?args.presentationRef:null}
    function presentationResult(value){const metadata=hostToolResultMetadata(value),open=metadata&&metadata[DASHBOARD_OPEN_METADATA_KEY];return open&&open.automatic===true&&open.scope==="conversation"&&validPresentationRef(open.presentationRef)?open.presentationRef:null}
    ...
    function acceptPresentationInput(value,source){if(presentationLinked)return;const ref=presentationInput(value);if(!ref)return;presentationInputRef=ref;document.documentElement.dataset.dashboardPresentationInputSource=source;syncPresentationDiagnostic()}
    ...
    function acceptPresentationResult(value,source){if(presentationLinked)return;const ref=presentationResult(value);if(!ref)return;presentationResultRef=ref;document.documentElement.dataset.dashboardPresentationResultSource=source;syncPresentationDiagnostic()}
    ...
    const automaticDashboardOpen=presentationLinked;
