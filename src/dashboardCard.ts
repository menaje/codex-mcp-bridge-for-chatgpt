...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 15;
...
<body>
  ...
  <script>
    ...
    function appendAgentBody(body,row,grouped=false){const head=node("div","row-head"),title=node("div","row-title",row.agentName||t["dashboard.jobFallback"]),state=node("span","status "+tone(row.status),statusLabel(row.status)),turn=latestTurn(row),rowKey=historyKey(row);head.append(title,state);body.appendChild(head);if(!grouped&&turn)body.appendChild(node("div","turn-title",turnActivityTitle(turn)));appendRowContext(body,row,grouped?"agent":"row");const meta=rowMeta(row);if(meta)body.appendChild(node("div","meta",meta));if(turn){body.appendChild(node("div","time",timeMeta(turn,row.status)));appendExecution(body,turn.execution);if(!grouped||turn.cancellation&&turn.cancellation.targetKind!=="activity")appendCancellation(body,turn.cancellation,rowKey+"\u0000current\u0000"+String(turn.cancellation&&turn.cancellation.requestedAt||""))}else if(row.bucket!=="active")body.appendChild(node("div","time",t["activity.lastChanged"]+" "+relativeTime(row.updatedAt)));if(shouldShowDashboardNextExecution(row.execution,turn&&turn.execution))appendExecution(body,row.execution,true);appendAgentHistory(body,row,turn)}
    function renderAgentRow(parent,row){const item=node("article","row"),marker=node("span","dot "+tone(row.status)),body=node("div","row-body");marker.setAttribute("aria-hidden","true");appendAgentBody(body,row,false);item.append(marker,body);parent.appendChild(item)}
    function renderActivityGroup(parent,group){const representative=group.rows[0];if(!representative)return;const item=node("article","row activity-group"),body=node("div","row-body"),head=node("div","row-head"),title=node("div","row-title",group.activityTitle||t["dashboard.jobFallback"]),agents=node("div","activity-agent-list"),activityCancellation=group.rows.map((row)=>latestTurn(row)).find((turn)=>turn&&turn.cancellation&&turn.cancellation.targetKind==="activity")?.cancellation;head.appendChild(title);body.appendChild(head);appendRowContext(body,representative,"activity");body.appendChild(node("div","activity-group-count",t["dashboard.agentCount"].replace("{count}",formatNumber(group.rows.length))));appendCancellation(body,activityCancellation,"activity\u0000"+group.activityKey+"\u0000"+String(activityCancellation&&activityCancellation.requestedAt||""));for(const row of group.rows){const agent=node("div","activity-agent");appendAgentBody(agent,row,true);agents.appendChild(agent)}body.appendChild(agents);item.appendChild(body);parent.appendChild(item)}
    function renderActivityRows(parent,rows){parent.replaceChildren();for(const group of groupDashboardRowsByActivity(rows))renderActivityGroup(parent,group)}
