...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 11;
...
export const DASHBOARD_CARD_RESOURCE_DESCRIPTOR = {
  ...
  description:
    "Read-only, status-first Codex runtime overview with project labels, verified local Codex thread links, best-effort GPT conversation links when a UUID-shaped host route candidate was captured, Agents, expandable retained turn history, and model/effort selections.",
...
<body>
  ...
  <script>
    ...
    function appendRowContext(parent,row){const context=node("div","row-context"),project=node("span","project-label",row.projectName||t["dashboard.unknownProject"]),codexUrl=safeCodexThreadUrl(row.codexThreadUrl),conversationUrl=safeConversationUrl(row.conversationUrl);context.appendChild(project);if(codexUrl){const link=node("a","conversation-link codex-session-link",t["dashboard.openCodexSession"]+" ↗");link.href=codexUrl;link.target="_blank";link.rel="noopener noreferrer";link.addEventListener("click",(event)=>openConversation(event,codexUrl));context.appendChild(link)}if(conversationUrl){const link=node("a","conversation-link",t["dashboard.openConversation"]+" ↗");link.href=conversationUrl;link.target="_blank";link.rel="noopener noreferrer";link.addEventListener("click",(event)=>openConversation(event,conversationUrl));context.appendChild(link)}parent.appendChild(context)}
    ...
    function renderAgentRow(parent,row){const item=node("article","row"),marker=node("span","dot "+tone(row.status));marker.setAttribute("aria-hidden","true");const body=node("div","row-body"),head=node("div","row-head"),title=node("div","row-title",row.agentName||t["dashboard.jobFallback"]),state=node("span","status "+tone(row.status),statusLabel(row.status)),turn=latestTurn(row),rowKey=historyKey(row);head.append(title,state);body.appendChild(head);if(turn)body.appendChild(node("div","turn-title",turnActivityTitle(turn)));appendRowContext(body,row);const meta=rowMeta(row);if(meta)body.appendChild(node("div","meta",meta));if(turn){body.appendChild(node("div","time",timeMeta(turn,row.status)));appendExecution(body,turn.execution);appendCancellation(body,turn.cancellation,rowKey+"\u0000current\u0000"+String(turn.cancellation&&turn.cancellation.requestedAt||""))}else if(row.bucket!=="active")body.appendChild(node("div","time",t["activity.lastChanged"]+" "+relativeTime(row.updatedAt)));if(row.execution&&row.execution.isCurrent&&(!turn||!turn.execution||!turn.execution.isCurrent))appendExecution(body,row.execution,true);const history=Array.isArray(row.history)?row.history:[],historyTotal=Math.max(history.length,Number(row.historyCount)||0);if(history.length){const key=rowKey,details=node("details","history"),summary=node("summary","history-toggle"),chevron=node("span","history-chevron"),label=node("span","history-label"),list=node("div","history-list");let previousActivityTitle=turn?turnActivityTitle(turn):null;chevron.setAttribute("aria-hidden","true");summary.append(chevron,label);history.forEach((historicalTurn,index)=>{const activityTitle=turnActivityTitle(historicalTurn);list.appendChild(renderHistoryTurn(historicalTurn,key+"\u0000history\u0000"+index+"\u0000"+String(historicalTurn.cancellation&&historicalTurn.cancellation.requestedAt||""),activityTitle!==previousActivityTitle));previousActivityTitle=activityTitle});details.append(summary,list);details.open=expandedHistories.has(key);syncHistorySummary(details,label,history.length,historyTotal);details.addEventListener("toggle",()=>{if(details.open)expandedHistories.add(key);else expandedHistories.delete(key);syncHistorySummary(details,label,history.length,historyTotal);scheduleSizeChanged(true)});body.appendChild(details)}item.append(marker,body);parent.appendChild(item)}
