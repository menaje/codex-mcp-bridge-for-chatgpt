...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 7;
...
export const DASHBOARD_CARD_RESOURCE_DESCRIPTOR = {
  ...
  description:
    "Read-only, status-first Codex runtime overview with project labels, best-effort GPT conversation links when a UUID-shaped host route candidate was captured, Agents, expandable retained turn history, and model/effort selections.",
...
export const DASHBOARD_CARD_CONTENT_METADATA = {
  ...
  "openai/widgetDescription":
    "Shows one read-only, status-first Codex overview derived from retained Codex Jobs, Agents, threads, interaction state, model/effort selection, and bounded App Server runtime evidence.",
...
<body>
  <main class="card">
    ...
    <div id="dashboard-content" hidden>
      <section class="counts" aria-label="Overview counts">
  ...
  <script>
    ...
    async function callTool(name,args){const compatibility=window.openai&&typeof window.openai.callTool==="function"?()=>window.openai.callTool(name,args):undefined;return callUiToolWithFallback(()=>standardToolCall(name,args),compatibility,{standardTimeoutMs:STANDARD_CALL_BUDGET_MS,compatibilityTimeoutMs:TOOL_CALL_TIMEOUT_MS,timeoutMessage:t["common.error"],shouldFallback:(error)=>!error||error.code!=="MCP_TOOL_CALL_DISPATCH_TIMEOUT"})}
    ...
    function consumeHostResult(metadataValue,toolOutput){const metadata=hostToolResultMetadata(metadataValue),candidate=privateView(metadata);if(candidate){render(candidate,false,1);return true}const normalized=normalizeHostToolResult(metadataValue);if(normalized&&normalized!==metadataValue){try{render(unwrap(normalized),false,1);return true}catch{}}if(toolOutput!==undefined){try{render(unwrap(toolOutput),false,1);return true}catch{}try{render(unwrap({structuredContent:toolOutput,_meta:metadata}),false,1);return true}catch{}}return false}
    ...
    function latestTurn(row){if(row.latestTurn&&typeof row.latestTurn==="object")return row.latestTurn;const terminal=["completed","failed","interrupted","cancelled"].includes(row.status),started=Date.parse(row.createdAt),ended=Date.parse(row.updatedAt),duration=terminal&&Number.isFinite(started)&&Number.isFinite(ended)?Math.max(0,ended-started):Math.max(0,Number(row.elapsedMs)||0);return{activityTitle:row.activityTitle||null,execution:row.execution,status:row.status,startedAt:row.createdAt,updatedAt:row.updatedAt,endedAt:terminal?row.updatedAt:null,durationMs:duration}}
    function timeMeta(turn,rowStatus){if(!turn)return"";const active=!turn.endedAt&&["running","input-required","approval-required","terminating","termination-failed","liveness-unknown","orphaned"].includes(rowStatus);if(active){const elapsed=turn.durationMs===null?t["dashboard.time.durationUnknown"]:t["dashboard.time.active"].replace("{duration}",formatDuration(turn.durationMs)),updated=t["dashboard.time.updated"].replace("{relative}",relativeTime(turn.updatedAt));return elapsed+" · "+updated}const duration=turn.durationMs===null?t["dashboard.time.durationUnknown"]:t["dashboard.time.duration"].replace("{duration}",formatDuration(turn.durationMs)),ended=t["dashboard.time.terminal"].replace("{status}",statusLabel(turn.status)).replace("{relative}",relativeTime(turn.endedAt||turn.updatedAt));return duration+" · "+ended}
    ...
    function openConversation(event,url){const api=window.openai;if(!api||typeof api.openExternal!=="function")return;event.preventDefault();try{const opened=api.openExternal({href:url,redirectUrl:false});if(opened&&typeof opened.then==="function")Promise.resolve(opened).catch(()=>openConversationFallback(url))}catch{openConversationFallback(url)}}
    function appendRowContext(parent,row){const context=node("div","row-context"),project=node("span","project-label",row.projectName||t["dashboard.unknownProject"]),url=safeConversationUrl(row.conversationUrl);context.appendChild(project);if(url){const link=node("a","conversation-link",t["dashboard.openConversation"]+" ↗");link.href=url;link.target="_blank";link.rel="noopener noreferrer";link.addEventListener("click",(event)=>openConversation(event,url));context.appendChild(link)}parent.appendChild(context)}
    ...
    function appendExecution(parent,execution,current=false){if(!execution)return;const value=executionText(execution),text=current?t["dashboard.execution.current"].replace("{execution}",value):value,badge=node("div","execution",text);badge.title=text;parent.appendChild(badge)}
    ...
    function renderHistoryTurn(turn){const item=node("div","history-turn"),head=node("div","row-head"),title=node("div","row-title",turn.activityTitle||t["dashboard.jobFallback"]),state=node("span","status "+tone(turn.status),statusLabel(turn.status));head.append(title,state);item.append(head,node("div","time",timeMeta(turn,turn.status)));appendExecution(item,turn.execution);return item}
    function renderAgentRow(parent,row){const item=node("article","row"),marker=node("span","dot "+tone(row.status));marker.setAttribute("aria-hidden","true");const body=node("div","row-body"),head=node("div","row-head"),title=node("div","row-title",row.agentName||t["dashboard.jobFallback"]),state=node("span","status "+tone(row.status),statusLabel(row.status)),turn=latestTurn(row);head.append(title,state);body.appendChild(head);if(turn)body.appendChild(node("div","turn-title",turn.activityTitle||t["dashboard.jobFallback"]));appendRowContext(body,row);const meta=rowMeta(row);if(meta)body.appendChild(node("div","meta",meta));if(turn){body.appendChild(node("div","time",timeMeta(turn,row.status)));appendExecution(body,turn.execution)}else body.appendChild(node("div","time",t["activity.lastChanged"]+" "+relativeTime(row.updatedAt)));if(row.execution&&row.execution.isCurrent&&(!turn||!turn.execution||!turn.execution.isCurrent))appendExecution(body,row.execution,true);const history=Array.isArray(row.history)?row.history:[],historyTotal=Math.max(history.length,Number(row.historyCount)||0);if(history.length){const key=historyKey(row),details=node("details","history"),summary=node("summary","history-toggle"),chevron=node("span","history-chevron"),label=node("span","history-label"),list=node("div","history-list");chevron.setAttribute("aria-hidden","true");summary.append(chevron,label);for(const historicalTurn of history)list.appendChild(renderHistoryTurn(historicalTurn));details.append(summary,list);details.open=expandedHistories.has(key);syncHistorySummary(details,label,history.length,historyTotal);details.addEventListener("toggle",()=>{if(details.open)expandedHistories.add(key);else expandedHistories.delete(key);syncHistorySummary(details,label,history.length,historyTotal);scheduleSizeChanged(true)});body.appendChild(details)}item.append(marker,body);parent.appendChild(item)}
    function renderAgentRows(parent,rows){parent.replaceChildren();for(const row of rows)renderAgentRow(parent,row)}
    ...
    async function reload(manual=false){if(busy||!mounted||!manual&&automaticRefreshDisabled)return;if(manual)automaticRefreshDisabled=false;setBusy(true);try{appendRequest=null;const result=await callTool("codex_dashboard_snapshot",{widgetInstanceId,limit:PAGE_LIMIT,terminalOffset:0,idleOffset:0});render(unwrap(result),false,2);lastRefreshAt=Date.now();automaticRefreshDisabled=false}catch(error){if(mounted)showError(error)}finally{appendRequest=null;setBusy(false)}}
    async function loadMore(bucket){if(busy||!mounted||!view)return;const page=bucket==="terminal"?terminalPagination:idlePagination;if(!page||!page.hasNext)return;setBusy(true);try{const nextOffset=page.offset+page.returned,args={widgetInstanceId,limit:PAGE_LIMIT,terminalOffset:bucket==="terminal"?nextOffset:0,idleOffset:bucket==="idle"?nextOffset:0};appendRequest={bucket,requestedOffset:nextOffset};const result=await callTool("codex_dashboard_snapshot",args);render(unwrap(result),false,2);lastRefreshAt=Date.now();automaticRefreshDisabled=false}catch(error){if(mounted)showError(error)}finally{appendRequest=null;setBusy(false)}}
    ...
    window.addEventListener("message",(event)=>{if(event.source!==window.parent)return;const message=event.data;if(!message||message.jsonrpc!=="2.0")return;if(message.method==="ping"&&message.id!==undefined){window.parent.postMessage({jsonrpc:"2.0",id:message.id,result:{}},"*");return}if(message.method==="ui/resource-teardown"){mounted=false;if(resizeObserver)resizeObserver.disconnect();cancelPending("Codex overview unmounted");if(message.id!==undefined)window.parent.postMessage({jsonrpc:"2.0",id:message.id,result:{}},"*");return}if(Object.prototype.hasOwnProperty.call(message,"id")&&pending.has(message.id)){const handler=pending.get(message.id);pending.delete(message.id);if(message.error)handler.reject(new Error(errorText(message.error)));else handler.resolve(message.result);return}if(message.method==="ui/notifications/host-context-changed"){const context=message.params||{};if(context.locale){hostLocaleTag=String(context.locale);if(localePreference==="auto")setLocale(hostLocaleTag)}return}if(message.method==="ui/notifications/tool-result"){try{render(unwrap(message.params),false,1)}catch{}}},{passive:true});
    window.addEventListener("openai:set_globals",(event)=>{const globals=event&&event.detail&&event.detail.globals||{};if(globals.locale){hostLocaleTag=String(globals.locale);if(localePreference==="auto")setLocale(hostLocaleTag)}consumeHostResult(globals.toolResponseMetadata||{},globals.toolOutput)});
    window.addEventListener("pagehide",()=>{mounted=false;cancelPending("Codex overview unmounted")});window.addEventListener("pageshow",()=>{mounted=true;if(view){syncDisclosure();syncLoadMoreControls();scheduleSizeChanged(true)}if(!standardBridgeInitialized)void beginStandardBridge();if(!automaticRefreshDisabled&&Date.now()-lastRefreshAt>1000)void reload()});document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"&&!automaticRefreshDisabled&&Date.now()-lastRefreshAt>30000)void reload()});
    standardBridgeReady=beginStandardBridge();setLocale(localeTag,false);consumeHostResult(initialResponseMetadata,window.openai&&window.openai.toolOutput);if(typeof ResizeObserver==="function"){resizeObserver=new ResizeObserver(()=>scheduleSizeChanged());resizeObserver.observe(document.documentElement);resizeObserver.observe(document.body)}void reload();
  </script>
</body>
</html>`;
