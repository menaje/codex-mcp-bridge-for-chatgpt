...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 26;
...
export const DASHBOARD_CARD_HTML_MAX_BYTES = 184 * 1_024;
...
${DASHBOARD_CONTROL_SCRIPT}
    ...
    async function selectStatus(status){if(problemMutationInFlight||!mounted||status===selectedStatus)return;hydrationEpoch++;appendRequest=null;setBusy(false);closeWorkDetails(false);resetProblems();selectedStatus=status;view=null;activeRows=[];terminalRows=[];idleRows=[];terminalPagination=null;idlePagination=null;lastRenderedAt=0;elements.activeList.replaceChildren();elements.terminalList.replaceChildren();elements.content.hidden=true;elements.updated.textContent="";syncStatusFilter();await reload(true)}
