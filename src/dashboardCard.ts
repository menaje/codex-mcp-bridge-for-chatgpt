...
export const DASHBOARD_CARD_CONTRACT_GENERATION = 25;
...
export const DASHBOARD_CARD_HTML_MAX_BYTES = 152 * 1_024;
...
export const DASHBOARD_CARD_RESOURCE_DESCRIPTOR = {
  ...
  description:
    "Codex status card with conversation-first defaults, an all-conversations view, nested Agents, retained turn history, and validated work controls.",
...
export const DASHBOARD_CARD_CONTENT_METADATA = {
  ...
  "openai/widgetDescription":
    "Shows Codex work and history from this conversation when records exist, otherwise all conversations. Switch scope at the top; both views use the same execution states and work controls.",
...
<body>
  <main class="card">
    ...
    <div id="dashboard-content" hidden>
      ...
      <button id="status-all" class="status-reset" type="button" data-i18n="dashboard.showAll" hidden></button>
  ...
  <script>
    const BUNDLES=${serializedUiTranslations(["common", "usage", "cancellation", "history.finite", "history.unlimited", "history.notice", "history.cleanup", "history.acknowledge", "history.archive", "history.restore", "history.started", ...DASHBOARD_TRANSLATION_KEYS, "activity.lastChanged", "activity.forceStop", "activity.forceConfirmTitle", "activity.forceConfirm", "activity.partialChanges", "activity.approve", "activity.approveSession", "activity.decline", "activity.answer", "activity.inputRequired", "activity.approval", "activity.optionalInput", "activity.openRequest", "activity.otherAnswer", "activity.yes", "activity.no", "activity.backgroundUnavailable", "question.gptHandles"])};
    ...
    function timeMeta(turn,rowStatus){if(!turn)return"";const active=!turn.endedAt&&["running","input-required","approval-required","terminating","liveness-unknown"].includes(turn.status),start=Date.parse(turn.startedAt);const elapsed=active&&Number.isFinite(start)?Math.max(0,Date.now()-start):turn.durationMs;const duration=elapsed==null?t["dashboard.time.durationUnknown"]:t["dashboard.time.duration"].replace("{duration}",formatDuration(elapsed));const relative=relativeTime(active?(turn.startedAt||turn.updatedAt):(turn.endedAt||turn.updatedAt));return[duration,active?t["history.started"].replace("{relative}",relative):relative].filter(Boolean).join(" · ")}
    function renderHistoryPolicy(policy){const element=document.getElementById("history-policy");element.hidden=!policy;if(!policy)return;const values=[(policy.retentionDays===0?t["history.unlimited"]:t["history.finite"].replace("{days}",formatNumber(policy.retentionDays))),t["history.notice"]];if(policy.lastCleanupAt)values.push(t["history.cleanup"].replace("{time}",new Intl.DateTimeFormat(localeTag,{dateStyle:"short",timeStyle:"short"}).format(new Date(policy.lastCleanupAt))).replace("{count}",formatNumber(policy.lastCleanupCount)));element.textContent=values.join("\n")}
    function appendHistoryControls(parent,row){const controls=row.historyControls;if(!controls)return;const actions=node("div","row-context history-actions");for(const [allowed,action] of [[controls.canAcknowledge,"acknowledge"],[controls.canArchive,"archive"],[controls.canRestore,"restore"]]){if(!allowed)continue;const button=node("button","",t["history."+action]);button.type="button";button.disabled=busy;button.addEventListener("click",()=>void changeHistory(row,action));actions.appendChild(button)}if(actions.childElementCount)parent.appendChild(actions)}
    async function changeHistory(row,action){if(busy||!mounted)return;setBusy(true);let failure=null;try{const rowKey=row.rowKey,expectedRevision=row.historyControls.revision,read=await callTool("codex_ui_read",{view:"history",rowKey,expectedRevision,widgetInstanceId});if(normalizeHostToolResult(read)?.isError)throw new Error(errorText(normalizeHostToolResult(read)));const proof=hostToolResultMetadata(read)["codex/historyControl@1"];if(proof?.rowKey!==rowKey||proof?.revision!==expectedRevision||!proof?.token)throw new Error(t["common.error"]);const result=normalizeHostToolResult(await callTool("codex_ui_history",{rowKey,expectedRevision,action,widgetInstanceId,token:proof.token,requestId:mutationId()},false));if(result?.isError||result?.structuredContent?.ok!==true)throw new Error(errorText(result))}catch(error){failure=error}finally{setBusy(false)}if(mounted){await reload(true);if(failure)showError(failure)}}
    ...
    function appendExecution(parent,execution,required=false){if(!execution&&!required)return;const text=execution?executionText(execution):t["dashboard.execution.unavailable"],badge=node("div","execution",text);badge.title=text;if(usesFastProcessing(execution))badge.appendChild(node("span","fast-mode","⚡ "+t["dashboard.execution.fast"]));parent.appendChild(badge)}
    ...
    function appendAgentBody(body,row,grouped=false,suppressIdleStatus=false,enclosingActivityTitle=null){const head=node("div","row-head"),title=node("div","row-title",row.agentName||t["dashboard.jobFallback"]),state=node("span","status "+tone(row.status),statusLabel(row.status)),turn=latestTurn(row),rowKey=historyKey(row);head.appendChild(title);if(!suppressIdleStatus||row.status!=="idle")head.appendChild(state);body.appendChild(head);if(!grouped&&turn)body.appendChild(node("div","turn-title",turnActivityTitle(turn)));appendRowContext(body,row,grouped?"agent":"row");const meta=rowMeta(row);if(meta)body.appendChild(node("div","meta",meta));if(turn){body.appendChild(node("div","time",timeMeta(turn,row.status)));appendExecution(body,turn.execution,true);if(!grouped||turn.cancellation&&turn.cancellation.targetKind!=="activity")appendCancellation(body,turn.cancellation,rowKey+"\u0000current\u0000"+String(turn.cancellation&&turn.cancellation.requestedAt||""))}else if(row.bucket!=="active")body.appendChild(node("div","time",[t["dashboard.time.durationUnknown"],relativeTime(row.updatedAt)].filter(Boolean).join(" · ")));appendAgentHistory(body,row,turn,enclosingActivityTitle);appendWorkControl(body,row);appendHistoryControls(body,row)}
...
${DASHBOARD_CONTROL_SCRIPT}
    async function enrich(args,epoch,pageRequest=null){try{const result=await callTool("codex_ui_read",{view:"dashboard",...args,enrich:true});if(mounted&&epoch===hydrationEpoch){const next=unwrap(result);enrichmentFailed=false;render(next,false,pageRequest)}}catch{if(mounted&&epoch===hydrationEpoch&&view){enrichmentFailed=true;paint(view)}} }
    async function reload(manual=false){if(busy||!mounted||!manual&&automaticRefreshDisabled)return;if(manual)automaticRefreshDisabled=false;const epoch=++hydrationEpoch;refreshFailed=false;setBusy(true);elements.message.textContent=t["common.loading"];elements.message.classList.remove("error");try{appendRequest=null;const args={widgetInstanceId,scope:selectedScope,statusFilter:selectedStatus,limit:PAGE_LIMIT,terminalOffset:0,idleOffset:0,enrich:false},result=await callTool("codex_ui_read",{view:"dashboard",...args});if(!mounted||epoch!==hydrationEpoch)return;render(unwrap(result));lastRefreshAt=Date.now();automaticRefreshDisabled=false;void enrich({...args,scope:selectedScope},epoch)}catch(error){if(mounted&&epoch===hydrationEpoch)showError(error)}finally{if(epoch===hydrationEpoch){appendRequest=null;setBusy(false)}}}
    async function loadMore(bucket){if(busy||!mounted||!view)return;const page=bucket==="terminal"?terminalPagination:idlePagination;if(!page||!page.hasNext)return;const epoch=++hydrationEpoch;refreshFailed=false;setBusy(true);try{const nextOffset=page.offset+page.returned,args={widgetInstanceId,scope:selectedScope,statusFilter:selectedStatus,limit:PAGE_LIMIT,terminalOffset:bucket==="terminal"?nextOffset:0,idleOffset:bucket==="idle"?nextOffset:0,enrich:false},pageRequest={bucket,requestedOffset:nextOffset};appendRequest=pageRequest;const result=await callTool("codex_ui_read",{view:"dashboard",...args});if(!mounted||epoch!==hydrationEpoch)return;render(unwrap(result),false,pageRequest);lastRefreshAt=Date.now();automaticRefreshDisabled=false;void enrich(args,epoch,pageRequest)}catch(error){if(mounted&&epoch===hydrationEpoch)showError(error)}finally{if(epoch===hydrationEpoch){appendRequest=null;setBusy(false)}}}
    ...
    async function selectScope(mode){if(!mounted||mode===selectedScope||mode==="conversation"&&!conversationAvailable)return;hydrationEpoch++;appendRequest=null;setBusy(false);closeWorkDetails(false);selectedScope=mode;view=null;activeRows=[];terminalRows=[];idleRows=[];terminalPagination=null;idlePagination=null;lastRenderedAt=0;expandedHistories.clear();expandedCancellations.clear();elements.activeList.replaceChildren();elements.terminalList.replaceChildren();elements.content.hidden=true;elements.updated.textContent="";syncScope();await reload(true)}
    ...
    async function selectStatus(status){if(!mounted||status===selectedStatus)return;hydrationEpoch++;appendRequest=null;setBusy(false);closeWorkDetails(false);selectedStatus=status;view=null;activeRows=[];terminalRows=[];idleRows=[];terminalPagination=null;idlePagination=null;lastRenderedAt=0;elements.activeList.replaceChildren();elements.terminalList.replaceChildren();elements.content.hidden=true;elements.updated.textContent="";syncStatusFilter();await reload(true)}
    for(const button of document.querySelectorAll("[data-status-filter]"))button.addEventListener("click",()=>void selectStatus(button.dataset.statusFilter===selectedStatus?"all":button.dataset.statusFilter));elements.statusAll.addEventListener("click",()=>void selectStatus("all"));
    ...
    function supersedeRequests(reason){hydrationEpoch++;appendRequest=null;setBusy(false);cancelPending(reason);if(!standardBridgeInitialized)standardBridgeAttempt=null}
    ...
    window.addEventListener("pagehide",()=>{mounted=false;supersedeRequests("Codex overview unmounted")});window.addEventListener("pageshow",()=>{mounted=true;const requiresFresh=Date.now()-lastRefreshAt>1000;if(view){syncStatusFilter();syncLoadMoreControls();scheduleSizeChanged(true)}if(!standardBridgeInitialized)void beginStandardBridge();if(requiresFresh){automaticRefreshDisabled=false;void reload()}});document.addEventListener("visibilitychange",()=>{if(document.visibilityState!=="visible"||Date.now()-lastRefreshAt<=30000)return;automaticRefreshDisabled=false;supersedeRequests("Codex overview refreshed");void reload()});
    window.addEventListener("online",()=>{if(!mounted||document.visibilityState!=="visible")return;automaticRefreshDisabled=false;supersedeRequests("Codex overview reconnected");void reload()});
