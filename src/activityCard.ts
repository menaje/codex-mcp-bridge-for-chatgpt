...
export const ACTIVITY_CARD_CONTRACT_GENERATION = 23;
...
export const ACTIVITY_CARD_HTML_MAX_BYTES = 144 * 1_024;
...
<body hidden>
  ...
  function refreshLabel(){return recoveryAction?t["activity.retry"]:historicalView()?t["activity.openLive"]:t["common.refresh"]}
  ...
  function setLocale(value,rerender=true){localeTag=String(value||"en").replaceAll("_","-");locale=resolveLocale(localeTag);t=BUNDLES[locale]||BUNDLES.en;document.documentElement.lang=localeTag;document.title=t["activity.title"];for(const item of document.querySelectorAll("[data-i18n]"))item.textContent=t[item.dataset.i18n]||BUNDLES.en[item.dataset.i18n];updateRefreshLabel();if(rerender&&snapshot)render(snapshot,true)}
	  ...
	  function mountedToolArgs(name,args){return MOUNTED_CARD_TOOLS.has(name)?Object.assign({},args,{widgetInstanceId}):args}
	  async function callTool(name,args,timeout=70000){const toolArgs=mountedToolArgs(name,args),response=window.openai&&typeof window.openai.callTool==="function"?await window.openai.callTool(name,toolArgs):(await standardBridgeReady,await rpcRequest("tools/call",{name,arguments:toolArgs},timeout));return normalizeHostToolResult(response)}
	  ...
	  function unwrap(value){const result=normalizeHostToolResult(value),metadata=hostToolResultMetadata(value),responseHostLocale=metadata.hostLocale||metadata["openai/locale"]||metadata["webplus/i18n"];if(responseHostLocale)hostLocaleTag=String(responseHostLocale);if(metadata.interactionControls)controls=metadata.interactionControls;if(result&&result.isError)throw new Error(errorText(result));return privateActivityOutput(metadata)||result&&result.structuredContent||parsedToolText(result)||result}
  ...
  function historicalView(){return Boolean(snapshot&&snapshot.mountedPresentation&&snapshot.mountedPresentation.kind==="historical")}
  ...
  function showMoreButton(){const more=actionButton(t["activity.showMore"],async()=>{more.disabled=true;viewLimit=Math.min(100,viewLimit+30);try{historicalView()?await reloadHistorical():await reload()}catch(error){showError(error);more.disabled=false}});more.className="disclosure";return more}
		  ...
		  function render(next,localeReady=false){
		    ...
		    if(next.watcherPolicy&&next.watcherPolicy.stopReason==="presentation-duplicate"){snapshot=null;historicalCorrelation=null;historyCursor=null;setCardVisible(false);return}
		    recoveryAction=null;refresh.disabled=false;setCardVisible(true);snapshot=next;renderWeeklyUsage(next.weeklyUsage);
		    if(next.mountedActivity)mountedActivityHint=next.mountedActivity;
		    if(next.mountedPresentation){mountedPresentationHint=next.mountedPresentation;historicalCorrelation=next.mountedPresentation.kind==="historical"?{jobId:next.mountedPresentation.jobId,requestId:next.mountedPresentation.requestId}:null}
	  ...
	  function rememberToolInput(input){if(!input)return;if(typeof input.prompt==="string")taskInvocation=true;if(typeof input.requestId==="string")taskInputRequestId=input.requestId}
		  ...
		  function consumeToolOutput(next){if(next&&next.feed&&Array.isArray(next.feed.active)){render(next);return true}const outputRequestId=next&&next.bridgeSession&&next.bridgeSession.requestId||next&&next.requestId;if(taskInputRequestId&&typeof outputRequestId==="string"&&taskInputRequestId!==outputRequestId){setCardVisible(false);return true}if(next&&next.kind==="task"&&typeof next.jobId==="string"&&typeof next.requestId==="string"&&typeof next.activityId==="string"){if(automaticBootstrapRequestId===next.requestId)return true;taskInvocation=true;const key="historical\u0000"+next.jobId+"\u0000"+next.requestId;if(taskBootstrapKey===key)return true;taskBootstrapKey=key;historicalCorrelation={jobId:next.jobId,requestId:next.requestId};mountedActivityHint=null;mountedPresentationHint=null;setCardVisible(false);void reloadHistorical().catch((error)=>{if(taskBootstrapKey!==key||!historicalCorrelation||historicalCorrelation.jobId!==next.jobId||historicalCorrelation.requestId!==next.requestId)return;if(shouldCollapseRehydrateError(error))setCardVisible(false);else showHydrationError(error,reloadHistorical)});return true}const presentation=next&&(next.bridgeActivity||next.activityTracking);if(!presentation){if(taskInvocation||next&&next.error)setCardVisible(false);return false}taskInvocation=true;if(typeof outputRequestId==="string")automaticBootstrapRequestId=outputRequestId;historicalCorrelation=null;const bootstrapEligible=presentation.shouldRenderActivityCard||AUTOMATIC_BOOTSTRAP_REASONS.has(presentation.renderReason);if(!bootstrapEligible){setCardVisible(false);return true}const activityId=presentation.activityId,cardGeneration=Number(presentation.cardGeneration),activityPresentationId=presentation.activityPresentationId,reservationOwnerId=presentation.jobId;if(typeof activityId!=="string"||!activityId||!Number.isInteger(cardGeneration)||cardGeneration<1||presentation.presentationKind!=="automatic"||typeof activityPresentationId!=="string"||!activityPresentationId){setCardVisible(false);return true}const key=activityPresentationId+"\u0000"+(typeof reservationOwnerId==="string"?reservationOwnerId:"");if(taskBootstrapKey===key)return true;taskBootstrapKey=key;mountedActivityHint={activityId,cardGeneration};mountedPresentationHint={kind:"automatic",activityPresentationId,...(typeof reservationOwnerId==="string"&&reservationOwnerId?{reservationOwnerId}:{})};setCardVisible(false);void reload().catch((error)=>showHydrationError(error,reload));return true}
  ...
  async function respondInteraction(row,agent,control,interaction,response,button){button.disabled=true;try{const payload=response.interactionDecision?{decision:response.interactionDecision}:{answers:response.interactionAnswers};await callTool("codex_interaction_respond",{requestId:mutationId(),jobId:control.jobId,expectedJobVersion:control.jobVersion,interactionId:interaction.interactionId,response:payload,card:cardProof()});await reload()}catch(error){showError(error);button.disabled=false}}
  ...
  async function requestFollowUp(kind,row,button){button.disabled=true;const prompt=localizedText(kind==="verify"?"activity.prompt.verify":"activity.prompt.retry",{activityId:row.activityId});try{await sendFollowUp(prompt);message.textContent=t["activity.followUpSent"]}catch(error){showError(error);button.disabled=false}}
	  ...
	  function showHydrationError(error,retry){recoveryAction=retry;if(!snapshot){renderWeeklyUsage(null);activityHeading.textContent=t["activity.currentActivities"];currentCount.textContent="";activeList.replaceChildren();groups.replaceChildren();updated.textContent="";setCardVisible(true)}refresh.disabled=false;updateRefreshLabel();showError(error,t["activity.loadFailed"])}
	  function cardProof(){const mountedActivity=snapshot&&snapshot.mountedActivity||mountedActivityHint,presentation=snapshot&&snapshot.mountedPresentation||mountedPresentationHint;if(!mountedActivity||!presentation||presentation.kind==="historical"||presentation.kind==="automatic"&&!presentation.activityPresentationId||presentation.kind!=="automatic"&&presentation.kind!=="explicit")throw new Error(t["common.error"]);return{activityId:mountedActivity.activityId,generation:mountedActivity.cardGeneration,presentation:presentation.kind==="automatic"?{kind:"automatic",activityPresentationId:presentation.activityPresentationId,...(presentation.reservationOwnerId?{reservationOwnerId:presentation.reservationOwnerId}:{})}:{kind:"explicit"}}}
		  async function reloadHistorical(){if(!historicalCorrelation)throw new Error(t["common.error"]);historyCursor=null;const correlation={...historicalCorrelation},next=unwrap(await callTool("codex_activity_rehydrate",{jobId:correlation.jobId,requestId:correlation.requestId,limit:viewLimit}));if(!historicalCorrelation||historicalCorrelation.jobId!==correlation.jobId||historicalCorrelation.requestId!==correlation.requestId)return;render(next)}
		  async function promoteHistorical(){const mountedActivity=snapshot&&snapshot.mountedActivity||mountedActivityHint;if(!mountedActivity)throw new Error(t["common.error"]);viewLimit=30;historyCursor=null;const next=unwrap(await callTool("codex_activity_snapshot",{card:{activityId:mountedActivity.activityId,generation:mountedActivity.cardGeneration,presentation:{kind:"explicit"}},limit:viewLimit,enrich:false}));historicalCorrelation=null;render(next)}
		  async function reload(){const card=cardProof(),args={card,limit:viewLimit,enrich:false};if(card.presentation.kind==="explicit"&&historyCursor)args.cursor=historyCursor;const next=unwrap(await callTool("codex_activity_snapshot",args));render(next)}
	  async function resumeCard(){if(!snapshot)return;historicalView()?await reloadHistorical():await reload()}
	  async function watch(){if(!mounted||!snapshot||watching)return;watching=true;try{const card=cardProof(),args={card,limit:viewLimit,afterVersion:snapshot.scopeVersion,waitMs:55000,enrich:false};if(card.presentation.kind==="explicit"&&historyCursor)args.cursor=historyCursor;const next=unwrap(await callTool("codex_activity_snapshot",args));failures=0;watching=false;if(mounted)render(next)}catch(error){watching=false;if(!mounted)return;failures++;if(failures>=5){message.textContent=t["activity.manualRefresh"];message.classList.add("error");return}const jitter=Math.floor(Math.random()*750);setTimeout(()=>void watch(),Math.min(30000,1000*Math.pow(2,failures))+jitter)}}
  ...
  async function dispatchHandoffs(){let claimed=[],card;try{card=cardProof();if(card.presentation.kind!=="automatic")return;const claim=unwrap(await callTool("codex_activity_handoff",{action:"claim-batch",outboxIds:snapshot.pendingHandoffs.map((event)=>event.outboxId),card}));claimed=claim&&Array.isArray(claim.events)?claim.events:[];if(claimed.length===0)return;const lookup=new Map(snapshot.pendingHandoffs.map((event)=>[event.outboxId,event])),ids=claimed.map((event)=>event.activityId),jobIds=claimed.flatMap((event)=>lookup.get(event.outboxId)&&lookup.get(event.outboxId).jobIds||[]),outboxIds=claimed.map((event)=>event.outboxId);const prompt=localizedText("activity.prompt.handoff",{origin:claim.origin,handoffDepth:claim.handoffDepth,handoffBatchId:claim.handoffBatchId,activityIds:ids.join(", "),jobIds:jobIds.join(", ")});await sendFollowUp(prompt);await callTool("codex_activity_handoff",{action:"delivered-batch",outboxIds,card})}catch(error){if(claimed.length&&card)await callTool("codex_activity_handoff",{action:"release-batch",outboxIds:claimed.map((event)=>event.outboxId),card}).catch(()=>{});showError(error)}}
	  ...
	  window.addEventListener("message",(event)=>{if(event.source!==window.parent)return;const value=event.data;if(!value||value.jsonrpc!=="2.0")return;if(value.method==="ping"&&value.id!==undefined){window.parent.postMessage({jsonrpc:"2.0",id:value.id,result:{}},"*");return}if(value.method==="ui/resource-teardown"&&value.id!==undefined){mounted=false;watching=false;if(resizeObserver)resizeObserver.disconnect();cancelPending("Activity card unmounted");window.parent.postMessage({jsonrpc:"2.0",id:value.id,result:{}},"*");return}if(value.id!==undefined&&pending.has(value.id)){const request=pending.get(value.id);pending.delete(value.id);value.error?request.reject(new Error(value.error.message||t["common.error"])):request.resolve(value.result);return}if(value.method==="ui/notifications/tool-input"){rememberToolInput(value.params&&value.params.arguments||value.params);return}if(value.method==="ui/notifications/host-context-changed"){if(value.params&&value.params.locale){hostLocaleTag=String(value.params.locale);if(localePreference==="auto")setLocale(hostLocaleTag)}return}if(value.method==="ui/notifications/tool-result"){try{consumeHostResult(value.params)}catch(error){showHydrationError(error,()=>consumeHostResult(value.params))}}},{passive:true});
	  window.addEventListener("openai:set_globals",(event)=>{const globals=event.detail&&event.detail.globals;try{if(!applyHostGlobals(globals)&&!snapshot)showHydrationError(new Error(t["activity.loadFailed"]),hydrateCurrentHost)}catch(error){showHydrationError(error,hydrateCurrentHost)}});
	  window.addEventListener("pagehide",()=>{mounted=false;watching=false;cancelPending("Activity card unmounted")});
	  window.addEventListener("pageshow",()=>{mounted=true;if(snapshot&&!watching)void resumeCard().catch(showError)});
	  document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"){mounted=true;if(snapshot&&!watching)void resumeCard().catch(showError)}});
		  refresh.addEventListener("click",()=>{const recovery=recoveryAction;if(recovery){refresh.disabled=true;message.textContent=t["common.loading"];message.classList.remove("error");Promise.resolve().then(recovery).catch((error)=>showHydrationError(error,recovery)).finally(()=>{refresh.disabled=false;updateRefreshLabel()});return}const action=historicalView()?promoteHistorical():reload();action.catch(showError)});
	  setLocale(localeTag);
	  standardBridgeReady=window.openai?Promise.resolve(false):initializeStandardBridge();
