...
<head>
  ...
  <style>
    ...
    .cancellation{display:grid;gap:7px;margin-top:8px;padding-left:11px;border-left:2px solid color-mix(in srgb,var(--danger) 32%,var(--border))}.cancellation-item{min-width:0}.cancellation-heading{font-size:11px;font-weight:650}.cancellation-reason{margin-top:2px;font-size:11px;line-height:1.45;overflow-wrap:anywhere}
...
<body hidden>
  <main class="card">
	...
	<script>
	  ...
	  ${serializeUiFunction(groupActivityIdleAgentsByActivity)}
  ...
  const expandedGroups={completed:false,idle:false,ended:false};
  ...
  function appendCancellations(parent,row){const entries=Array.isArray(row.cancellations)?row.cancellations.filter((entry)=>entry&&typeof entry.reason==="string"&&entry.reason.trim()):[];if(!entries.length)return;const list=node("div","cancellation");for(const entry of entries){const item=node("div","cancellation-item"),meta=[];if(entry.targetKind==="job"&&entry.agentName)meta.push(displayAgentName(entry.agentName));meta.push(t["cancellation.target."+entry.targetKind]||String(entry.targetKind||""));const requestedAt=new Date(entry.requestedAt);if(Number.isFinite(requestedAt.getTime()))meta.push(new Intl.DateTimeFormat(localeTag,{dateStyle:"short",timeStyle:"short"}).format(requestedAt));item.append(node("div","cancellation-heading",cancellationHeading(entry)),node("div","meta",unique(meta).join(" · ")),node("div","cancellation-reason",entry.reason));list.appendChild(item)}parent.appendChild(list)}
