...
describe("state schema 21 completion delivery after asynchronous-execution normalization", { timeout: 15_000 }, () => {
  it("upgrades schema 19 without losing request dedupe identity or terminal results", () => {
    ...
    expect(current.prepare("PRAGMA table_info(jobs)").all())
      .not.toContainEqual(expect.objectContaining({ name: "execution_mode" }));
    current.close();
    expect(existsSync(`${file}.pre-v19-to-v21.sqlite`)).toBe(true);
  });
  ...
  it("creates the current schema directly and matches an authentic schema 18 upgrade", () => {
    ...
    expect(fresh.schemaVersion).toBe(21);
    fresh.close();
    ...
    const backup = `${upgradedFile}.pre-v18-to-v21.sqlite`;
    expect(existsSync(backup)).toBe(true);
    expect(statSync(backup).mode & 0o777).toBe(0o600);
    expect(readSchemaVersion(backup)).toBe(18);
    expect(readdirSync(root).filter((name) => name.includes("pre-v18-to-v21"))).toHaveLength(1);
  ...
  it("upgrades the published schema 3 fixture through every supported checkpoint", () => {
    ...
    expect(existsSync(`${file}.pre-v3-to-v21.sqlite`)).toBe(true);
    expect(readSchemaVersion(`${file}.pre-v3-to-v21.sqlite`)).toBe(3);
    ...
    expect(readdirSync(root).filter((name) => name.includes("pre-v3-to-v21"))).toHaveLength(1);
  });
  ...
  it("keeps the original recovery point when a multi-checkpoint upgrade resumes", () => {
    ...
    const originalBackup = `${file}.pre-v3-to-v21.sqlite`;
    ...
    expect(existsSync(`${file}.pre-v18-to-v21.sqlite`)).toBe(false);
