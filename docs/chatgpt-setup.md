...
6. Verify seven app-only tools: `codex_ui_read`, `codex_update_settings`, `codex_question_action`, `codex_ui_stop`, `codex_ui_history`, `codex_ui_problem`, and `codex_interaction_respond`. The 12 retained app-only descriptors are separately marked compatibility; total discovery is 31 during migration. Operator diagnostics and recovery are opt-in.
...
Then deploy/restart the bridge before selecting **Refresh** on the ChatGPT plugin detail screen. The server must already serve the newly advertised current URI and every identity selected for compatibility, with its original metadata, required presenter, and app-only callers. The generated lock contains only current active cards and revisions selected by `ui-release-catalog.json`; accumulated development intermediates remain outside the package. The [UI release compatibility policy](ui-release-compatibility.md) defines the provenance and retirement rules. Generation support alone does not establish published-release compatibility.
...
Agent lifecycle is separate from turn and Activity lifecycle. A terminal turn returns the Agent to `idle` and releases its active Activity assignment while preserving history. Model-visible `codex_agent` provides only one discriminated `operation`: `rename` (with `name`). Schema 18 restores legacy archived Agents while preserving their identity, thread graph, Activity/project links, current Job state, results, review state, settings and timestamps; it performs no replay, resume or conversation load. Agent archive/restore and the 30-day automatic archive policy are removed. Retained archive/restore requests return `AGENT_ARCHIVE_REMOVED` without mutation. Exceptional assignment detach is an operator-enabled private recovery operation. There is no GPT-facing permanent delete.
...
described below remains available for the exact v0.3.0 and deployed-development
identities selected under the [UI retirement policy](ui-release-compatibility.md#activity-card-retirement).
Other development generations are implementation history outside the release
package and are not compatibility commitments.
...
New work does not open Activity cards. Keep the current GPT response active using bounded input waits and retrieve each terminal Job result with an exact query. Native Codex alerts remain independent. Existing mounted old cards retain their original completion handoff only during the migration window. The active-response completion flow and original controls were accepted in #69; a new wake mechanism for an already-ended GPT response is outside that completed scope. See [completion-flow acceptance](card-tools.md#completion-flow-acceptance) and the remaining device checks in the [evaluation plan](chatgpt-evaluation.md).

When the user asks for the Codex status card, call `codex_dashboard`. The current generation-29 card keeps the generation-23 scope choice: it starts with this GPT conversation if it has retained Activity or Job records, including completed history; otherwise it starts with all conversations. The **This conversation / All conversations** buttons switch the scope and each switch loads that scope separately before returning to the summary. The chosen scope survives explicit refresh and restoration of the mounted card. A new cold mount makes the initial choice again. Scope filtering happens on the server before counts and pagination; see [Card tools](card-tools.md). Opening and refreshing the overview is read-only; deliberately opened approval/input request details offer validated user controls in this personal, single-user bridge; there is no Dashboard operator flag to enable. “All” means scopes currently known from retained Activities, live Jobs, result-free archived Job summaries, Agents, or tracked threads—not the account's complete ChatGPT history. Run history defaults to 30 days with a configurable retention notice; see [Execution history](work-history.md). The default result-bearing Job window is six hours/100 Jobs, the Dashboard reads at most 10,000 archived summaries, and the tracked-thread registry defaults to 1,000 entries. The public result is a redacted aggregate fallback so a non-UI host still receives useful status, but it carries only locale app metadata and no full Dashboard view. The UI starts cold in loading state, renders a fresh structural app-only `codex_ui_read` (`view: "dashboard"`) without waiting for runtime or usage probes, and then applies a bounded enriched snapshot; it does not consume the conversation's cached initial tool result or later host replays of that result. Generation 29 initially requests a history-free status index explicitly marked complete with `statusRowsComplete: true` and hides all row lists. Status buttons classify that index locally. Opening run history requests its stored 12-row pages separately; omitting the optional status/history fields preserves active/recent/idle pages for immutable older cards. Larger project/conversation compatibility pages are computed only when an older card supplies those offsets. A mounted recovery call may omit conversation metadata, while any supplied host or compatibility scope is still validated.

The Dashboard reads on initial mount, the Refresh button, scope changes, stored-history pagination, and reconciliation after a direct control. Page restoration, visibility return, network recovery, and retained-state notifications do not reread Dashboard data. They repaint the loaded view or restore the UI transport only. Until the first fresh structural snapshot succeeds the card shows loading rather than cached or empty counts; an initial failure asks the user to retry with the same card’s Refresh button and never falls back to the original tool response. A failed later refresh leaves the last view and timestamp visible with a localized warning until explicit Refresh succeeds. Obsolete in-flight responses still cannot replace a newer view or cross a scope change. Cold mounts continue to require a fresh snapshot and do not render a cached conversation tool result. During cold startup, the compatibility bridge can serve structure immediately while standard MCP Apps initialization proceeds. Structural data is requested with `enrich: false`; a second `enrich: true` request belongs to that same explicit refresh and cannot hide or invalidate the structural result when it fails. Opening history or loading another history page uses `enrich: false`. Runtime inspection never calls `thread/resume` to populate a card. Retained generations 18–23 use the fixed `active → recent terminal → idle` order and group every section by opaque Activity identity with Agents nested below. The idle heading is explicitly **Recent Activity**, because an idle Agent has released its current assignment; Agents with no retained Activity are grouped under **No recent Activity**. All three legacy pages preserve whole Activity groups, so the nominal Agent-row limit may be exceeded when one Activity contains more Agents. Project and GPT conversation context appear once on the Activity; every Agent keeps the actual model/reasoning effort for its latest turn. Up to 12 older turns suppress the enclosing Activity title, retain distinct Activity-ID boundaries even when visible titles match, and always show their own execution evidence or an unavailable label. Generation 24 merges recorded turns into one run-history view and hides idle Agents with no recorded turn. Generation 29 leaves all row sections hidden until a status or history button is selected; older retained turns start collapsed under each Agent, and explicit refreshes preserve those disclosures without `localStorage`.

The mounted snapshot performs bounded read-only App Server checks for at most 200 recently updated Agents, with a 1.5-second per-Agent timeout and a six-second overall budget. Agents without a retained latest Job are included only when the upstream provides a non-loading thread probe; otherwise they are reported as skipped. Timed-out and deadline-deferred probes are also skipped/unknown, and the snapshot does not load a `notLoaded` historical thread merely for the overview. **Tracked projects** counts active, non-deleted project registrations rather than distinct project keys on the visible rows. An active but temporarily unavailable registration remains counted; archived/deleted registrations and the unassigned bucket do not. Historical rows admitted under an archived or deleted registration remain visible. The snapshot does not long-poll, consume Activity watcher admission, own completion handoff, or acquire a control lease. ChatGPT cannot update a dormant historical card while the host has not mounted it, so reopening that overview conversation is the expected refresh path. Refresh is a retained-state snapshot rather than a live health check of every historical thread.

Dashboard state must remain Codex-runtime-only: exact Job running/terminal/termination state, tracking liveness, Agent lifecycle, pending Codex input/approval interactions, and confirmed App Server background-terminal counts. Do not map Activity lifecycle, waiting, verification, completion handoff, or GPT's goal-completion judgment into these labels. **Codex turn completed** means exactly Job status `completed`; failed, interrupted, and cancelled remain separate terminal outcomes. **Running / Response needed / Issues** are the current card and native menu-bar summary. Input and approval waits count once per Agent under response needed and do not mark menu-bar health as broken. Issues count unresolved runtime inspection, disconnection, and termination problems, excluding normal waits and finished failures. Failed and interrupted executions remain in Failure history for their configured retention period, and a later retry does not hide an earlier execution there. Retained clients without the current Problems query keep the former seven-day, latest-per-Agent compatibility projection. Schema 18 restores legacy archived Agents before these views are served. Counts and locally filtered rows cover the same full selected scope, independent of the run-history page limit. Remaining background terminals appear separately from running Codex turns. Current work and run history group by Activity context with their saved Agent names nested below. Background processes have a separate conditional filter, terminating work retains its badge, and unknown/deferred runtime information remains explicit. The current/latest turn retains the exact effective model/reasoning-effort selection when known. Effort values use canonical lowercase catalog text in the card, menu bar and both Settings surfaces. A separately labelled next-run setting appears only when its model, effort, effective Fast tier, or reroute differs from the latest actual turn. Older calls for that same Agent appear only inside its expandable history. An idle Agent may additionally show its labeled current tracked-session selection; that value is never substituted for an unknown historical turn. Old archived summaries without a retained start time report **Work time unavailable** rather than zero. Active timing uses the duration captured by the last explicit snapshot and renders it once. Terminal and idle retained timing is Job start-to-terminal work time when known plus time since the exact outcome; absolute start, update, and end timestamps are not displayed. A reported runtime model reroute is shown as `selected → rerouted`, with the admission-time effective effort retained. The card never prints the session identifier, thread identifier, or compatibility alias as text. The bridge can retain a validated `codex://threads/<uuid>` candidate for the native macOS **Open Codex conversation** action, but the ChatGPT Dashboard does not render it because card-host navigation does not reliably open custom app schemes. Native availability still requires an exact retained tracked App Server session whose creation-time visibility bit permits it; the exact current thread wins over its matching session-tree UUID so forks open the correct descendant. Separately, OpenAI defines `openai/session` as an anonymized correlation value rather than a navigable-route guarantee. For a UUID-shaped host value, the bridge keeps a bounded private mapping and exposes one best-effort **Open conversation** link in each Activity context. GPT navigation is independent of the Codex-app visibility setting. Arbitrary values are not persisted, and scopes retained before capture cannot be backfilled. The ChatGPT route is not probed. The Dashboard CSP allows only `https://chatgpt.com` for `window.openai.openExternal`, and the anchor remains the fallback. Apart from those validated private route targets, the view contains hashed conversation, project, Activity, and row keys plus display context only—never raw Job, Activity, Agent, thread, worker, or process IDs; paths; prompts; results; errors; commands; or controls. User-defined project/Agent/Activity display labels and the navigation links can reveal cross-conversation task context, so keep this overview on the single trusted user's connection. A widget-instance UUID is correlation rather than authentication.

### Retained Activity cards during migration

The following presentation and handoff behavior describes existing saved cards
only. It is not an instruction to create a new Activity card. Current approval/input request details use the global card and app-only
`codex_ui_read` and `codex_interaction_respond` contracts. The current Dashboard
has no work-management or stop panel; `codex_ui_stop` remains only for retained
immutable cards. See
...
Only the newest confirmed mounted compact-monitor presentation owns the scope live watch and completion handoff. When a newer presentation confirms its first snapshot lease, the previous compact card receives a normal `presentation-superseded` stop result, retains its last snapshot, and releases its watcher slot without a retry loop. Explicit full-history cards use separate admission (up to three concurrent explicit watchers per scope alongside the one compact owner) and do not claim automatic completion handoff. `openai/widgetSessionId` identifies only the mounted widget instance and is not authorization by itself; every control revalidates the exact card lease and target ownership. Handoff discovery exposes batch actions only and requires the newest compact card proof. UI-bearing Activity tools use strict private/app-only view contracts; bootstrap remains only for immutable older resources. The selected content-hashed deployed Activity resource uses UI generation 30 and the closed private metadata envelopes. It paints structural data immediately, then requests bounded enrichment before dispatching the independent long watch. One-shot historical and restored-explicit cards may also request bounded enrichment without acquiring a watcher, handoff owner, control lease, or mutation authority. Its retained behavior bounds compatibility and standard calls, retries cold rehydration, ignores stale async responses, coalesces rapid refresh clicks, and leaves the last structural data visible with a localized warning when refresh or enrichment fails. Activity lifecycle stays on its heading and each nested Agent keeps the latest actual model/reasoning effort with turn state, work time, pending interactions, and Job-target cancellation explanation. Full-history pages keep the existing de-duplication rules. Its Force Stop action calls `codex_activity_job_cancel`, which validates the live widget lease, current presentation/card proof, exact Job version, target ownership, and cancellation UUID. The catalog also retains the original non-hashed v0.3.0 Activity identity and its legacy tool contract. Reservations and ownership are in-memory; after bridge restart the first valid supported card to reconnect safely re-establishes ownership.
...
App Server may leave a background terminal after the turn itself completes. While a Job is active, its owning retained Activity card exposes **Force-stop Agent work** and withholds background-process termination. After the Agent becomes idle, that Activity card can separately show the remaining-process count and **Stop background processes** action. The current Dashboard has neither control. Background-process termination calls the app-private, destructive `codex_background_process_terminate` tool. The bridge revalidates the mounted Activity-card lease, exact Agent version, current App Server thread, fresh terminal inventory, and absence of an active turn before calling `thread/backgroundTerminals/terminate`; Agent archive is no longer a supported operation, and this is not job force-stop. Every supported Activity resource calls this dedicated tool; the former Agent-operation shortcut has expired.
...
14. create schema 19 directly, upgrade authentic schema-18 and published schema-3 fixtures, and confirm identical final DDL; preserve active work, execution contexts, pending input/delivery/cancellation/steering/recovery/hold state and Job receipts, allow only declared invalid legacy-project contexts to disappear, inject a late rebuild failure, retry from the single original private backup, and verify two restarts are idempotent; also retain the schema-17-to-18 archived-Agent restoration assertions and confirm a retired archive/restore request fails without mutation;
...
- `AGENT_ARCHIVE_REMOVED`: refresh a cached card or tool definition. Agent archive/restore is retired, and the rejected request does not change Agent state.
