...
6. Verify six app-only tools: `codex_ui_read`, `codex_update_settings`, `codex_question_action`, `codex_ui_stop`, `codex_ui_history`, and `codex_interaction_respond`. The 12 retained app-only descriptors are separately marked compatibility; total discovery is 30 during migration. Operator diagnostics and recovery are opt-in.
