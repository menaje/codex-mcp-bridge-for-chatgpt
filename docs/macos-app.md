...
Changing the Tunnel ID marks the managed tunnel profile for recreation. A
separate **Tunnel profile repair** action can rebuild that profile from the
current dotenv after active work drains. It does not change SQLite, projects,
Settings, Codex authentication, or the dotenv.
