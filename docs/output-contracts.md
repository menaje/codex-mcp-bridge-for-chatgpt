...
## Generation 1 private Dashboard envelope and generation 15 UI
...
The current Dashboard descriptor requires generation 15 at the content-hashed URI recorded in `ui-manifest.lock.json`. Retained immutable Dashboard resources remain registered according to the lock-file compatibility policy; generation-4–6 offset calls receive their conditional grouping projections.

The minimum Activity generation for new descriptors is 12. The current immutable resource is generation 18 at the content-hashed URI recorded in `ui-manifest.lock.json`. Every retained immutable Activity URI from generations 7–18 remains registered and refreshes through app-only snapshot/watch paths. Retained HTML assets are not rewritten or deleted when the minimum advances. UI generation 18 continues to validate the generation-11 private bootstrap/view envelopes above, labels Activity rows only with their own lifecycle, and keeps turn state plus each pending interaction under the owning Agent.
