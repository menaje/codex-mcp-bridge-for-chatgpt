...
| `codex_settings` | Thin `{kind: "settings", opened: true}` acknowledgment; no snapshot or catalog lookup | The public result contains locale metadata but no editor view. App-only `codex_ui_read` (`view: "settings"`) is the current card's fresh full editor source; `codex_update_settings` returns the committed full editor view after a mutation. Both app-only results retain a validated `_meta["codex/settingsView"]` compatibility copy for older cards |
...
| `codex_models` | Current policy-allowed models, efforts, service-tier support, upstream descriptions and freshness. Explicit input `contractVersion: "2"` also returns that version and `selectionMode` from the same settings snapshot; omitted version preserves the exact legacy shape. Disabled Ultra choices are absent even when retained in Settings; a suspended explicit policy returns `models: []` and an explanatory bridge-policy `warning`. No Priority preference, ranking or fallback selection. See [source and policy semantics](model-selection.md) | None |
...
The server also supplies `_meta["codex/activityScopeId"]` with the exact scope resolved for the original Activity read. Generation 26 retains it in the mounted iframe and includes it in subsequent snapshot/rehydrate requests, including enrichment and watches, when a host omits conversation metadata. Supplied host identity still takes precedence over this compatibility routing value. It is absent from the closed public and app `structuredContent` schemas, so cached descriptors remain valid. A card with neither host metadata nor a retained scope still cannot infer a scope from Activity IDs.

Generation 26 retries interrupted structural and enrichment reads up to three times, suppresses obsolete responses, and keeps at most one outstanding watch per mounted widget across refresh and page re-entry. A failed refresh resumes live watching; repeated watch failures back off to a thirty-second retry interval instead of permanently stopping updates. Historical enrichment retries remain one-shot and never acquire a watch or control lease.
...
`_meta["codex/dashboardView@1"]` is a closed, 512-KiB-bounded snapshot envelope with purpose `bridge-wide-read-only-hydration`. The current public `codex_dashboard` result does not emit this envelope; only the mounted app-private snapshot does. Its view declares `coverage: "bridge-known-retained"`, a closed `enrichment` state/counter object, aggregate retained-state counts, confirmed background-process totals, explicit unknown/skipped runtime-probe counts, a history-free status index explicitly marked complete with `statusRowsComplete: true`, optional active/recent/idle history pages, and the UI locale preference. `historyIncluded` distinguishes a summary/status response from a paged history response. Each representative row is limited to opaque hashed row/project/conversation keys, a compatibility conversation alias, optional validated Codex and best-effort ChatGPT route candidates, display context, one Codex-runtime status, a presentation bucket, timestamps, elapsed time, an integer background-process count, and an optional effective execution selection. The stable row key is derived privately from Agent identity—or from Job identity for an unassigned Job—and is never rendered. It lets the UI evict a stale in-mount copy when the representative changes bucket or display text. The project key is derived from the private immutable project identity when available, with a normalized label or one unassigned bucket only as a compatibility fallback; source identities and paths never leave server state.

Generation 3 added `latestTurn` plus older retained `history` entries without Agent, Job, Activity, thread, or execution identifiers. Generations 4 and 5 added conversation and project grouping pages. Generation 7 introduced the fixed `active → recent terminal → idle` order. Generation 12 makes active and recent presentation Activity-first: each row carries an opaque hashed `activityKey`, same-title Activities remain distinct, and matching Agent rows render below their Activity. Generation 13 removes initial tool-result hydration: a cold mount keeps the content hidden behind loading, ignores cached tool-result notifications/globals, and first renders only after a fresh `codex_ui_read` (`view: "dashboard"`) succeeds. Generation 14 carries the opaque Activity key into retained turns and pages active/recent rows on whole-Activity boundaries. A page limit remains an Agent-row target and may be exceeded by one indivisible Activity. It also invalidates a mounted view before a page re-entry older than one second or a visibility return older than thirty seconds, so a failed fresh load cannot repaint the discarded stale rows. Generation 15 removes the Agent-derived Activity-heading status and renders every Codex turn state only on its owning Agent row. Generation 16 splits structural data from optional runtime/usage enrichment and embeds only card-specific translations. Generation 17 applies Activity-first rendering and whole-Activity paging to idle Agents as well, labels those groups as latest rather than current Activity, and groups Agents with no retained Activity by conversation/project fallback. Generation 18 keeps historical and next-run execution selections on each owning Agent instead of hoisting identical values to the Activity. Same-Activity history suppresses the already visible title; an Activity-ID change still creates a boundary, but a repeated visible title is replaced with **Previous Activity** rather than printed again. An ordinary in-place refresh failure after one successful snapshot still retains only that mounted iframe's last successful view; an enrichment failure retains the newly rendered structural view. Normal current snapshots omit the larger legacy group pages. The server emits them only when an immutable generation-4–6 card sends `conversationOffset` or `projectOffset`, so older resources can still refresh without imposing their payload cost on new cards. Through generation 23, recent and idle pages initially target 20 representative Agents and append later pages through **Show more**; generation 24 uses one run-history page instead. A response whose offset was clamped replaces that bucket instead of merging incompatible pages, and current active/terminal evidence evicts stale in-mount copies. The legacy idle section starts collapsed on each fresh card and retains its disclosure state through ordinary mounted refreshes; generation 24 removes that section. Generation 28 starts at the summary, adds the complete `statusRows` index (`statusRowsComplete: true`), and sets `includeHistory: false` until the user opens run history. Its status buttons never call the server. Generation 29 matches the native menu's single selected panel, 12-row history pages and `status → actual execution → snapshot/recorded time → older history` order. It shows **Next run settings** only when the current model, canonical lowercase effort, effective Fast tier or reroute differs from the latest actual turn. Page, visibility, and network restoration repaint retained state without a Dashboard read; explicit Refresh, scope changes, history pagination, and direct-action reconciliation remain read points.

Within every status section, the Activity title heads the group and Agent display names head its nested rows. Idle uses the latest retained Activity as historical context; it does not recreate a current assignment. Result-bearing Jobs retain full Dashboard turn data while ordinary retention pruning reduces the variable Job payload to a result-omitted marker. Formal Job columns retain correlation, terminal state, timestamps, Agent/backend context, and pinned execution context. The bounded summary retains only effective execution selection, usage, and uncertain-response review state when present. It never retains prompt, result, error, command, or public-event bodies for this purpose. Each current and retained turn may carry the same opaque `activityKey` used for grouping, so same-title Activities remain distinct and the current Activity title is not repeated in its own expanded Agent history. The Dashboard reads at most 10,000 archived summaries, pages 12 representative Agents at a time, sends at most 12 older turns per visible Agent, and reports the full history count. Every visible actual turn renders its own execution evidence. Effort values are trimmed, canonical lowercase catalog values in Dashboard and Settings; field labels and descriptions remain localized. Pre-generation-7 summaries can have null start/duration and no execution, so the UI shows work time unavailable and **Model · reasoning unavailable** rather than substituting the current session selection as historical fact. An idle or recovery row may separately expose its current tracked-session selection with `isCurrent: true`, labelled as next-run settings only when it differs from the last actual turn.

Retained history starts collapsed and expands below the same Agent without exposing an Agent ID or display alias; its in-memory disclosure state survives ordinary snapshot refreshes in the mounted iframe. Session, thread, and compatibility identifiers are never printed as text. A validated `codex://threads/<uuid>` candidate remains available to the native macOS client for **Open Codex conversation**, but the ChatGPT Dashboard does not render it because the card host's external navigation is browser-oriented and does not reliably open custom app schemes. The exact selected App Server thread must match a retained tracked session, and that session's creation-time visibility bit controls native availability; the current preference is a compatibility fallback only when the matching legacy session lacks the bit. Missing, mismatched, hidden, non-App-Server, or other value shapes are omitted. The exact thread UUID takes precedence over the matching session-tree UUID because forked threads can share the latter. OpenAI separately defines `openai/session` as an anonymized correlation value, not a navigable-route guarantee. When a UUID-shaped host value is captured, every Activity group renders one best-effort **Open conversation** link in its context. GPT navigation is independent of the Codex-app visibility preference. The ChatGPT route is not probed, and previously retained ChatGPT scopes cannot be backfilled without the deliberately unretained raw host value. The Dashboard CSP permits only `https://chatgpt.com` for `window.openai.openExternal`, while anchors remain the fallback. Active timing shows only the start-to-snapshot work time, so an unrefreshed row does not keep aging on screen. A terminal or idle retained turn shows start-to-terminal work time when known plus time since that exact outcome; absolute start, status-update, and end timestamps are not rendered. Model selection renders as `model · effort`, or `selected → rerouted · effort` when Codex reports a reroute. The fresh-snapshot decoder accepts private metadata as a raw map or inside complete `mcp_tool_result` or `call_tool_result` wrappers, including JSON-encoded nested results.

The status mapper consults only retained Job state and tracking liveness, pending Codex interactions, Agent lifecycle, and bounded read-only App Server thread/background-terminal evidence. Model/effort presentation does not participate in status calculation. Activity lifecycle, waiting, verification, completion handoff, and GPT completion judgment are neither fields nor inputs to that mapper. `completed` means exactly a completed Job; failed, interrupted, and cancelled remain distinct. For snapshots without a `problems` query, attention is deduplicated by Agent from the latest retained outcome. Current clients use the per-execution review collection described below.

The coverage set is the union of conversation scopes still known from live Jobs, bounded archived summaries, Agents, or tracked threads; it is not the ChatGPT account's complete history. `counts.trackedProjects` is independent of that row coverage: it counts active, non-deleted project registrations, includes an active registration whose folder is temporarily unavailable, and excludes archived/deleted registrations plus the unassigned compatibility bucket. Retained rows keep the immutable project ID and pinned execution cwd; their display name is joined from the current project row, including its tombstone after registration deletion. Unlinked rows have no project name. The public `codex_dashboard` tool needs no capability flag, returns a static opening acknowledgment with locale-only app metadata, does not read weekly usage, and defers App Server probing. App-only `codex_ui_read` (`view: "dashboard"`) verifies mounted-widget correlation, validates any supplied host or compatibility scope, and permits mounted recovery without scope metadata because web and desktop hosts can omit it during remount. Generation 16 explicitly sends `enrich: false` to read only structural bridge state, uses an immediately available ChatGPT compatibility call for cold structure while standard MCP Apps initialization proceeds, then requests `enrich: true`. Omission preserves the enriched response expected by immutable older cards. Generations 28–29 also send `statusFilter: "all"` and `includeHistory: false` for summary/status reads. After the first fresh snapshot, an ordinary in-place refresh failure retains that last snapshot and timestamp until explicit refresh. Current cards do not automatically retry or invalidate Dashboard data on page, visibility, network, or retained-state events.

Enrichment considers as many as 200 App Server Agents, independently of the currently displayed page. Visible/active candidates receive liveness inspection, while the remaining candidates use the loaded-thread-only terminal inventory needed to discover an off-page background process without resuming historical threads. The two candidate classes use independent worker pools so slow liveness reads cannot starve background-process discovery. It uses at most eight workers, a 1.5-second per-probe timeout, a six-second overall runtime budget, and a separate 1.5-second weekly-usage timeout. A worker whose uncancellable request times out launches no further request. Confirmed observations are fresh-cached for five seconds and retained as last-known evidence for 15 minutes using a backend/thread plus Agent/Job-generation stamp; a successful background-process mutation invalidates its thread entry. Weekly usage is fresh-cached for one minute and retained for 30 minutes. Structural snapshots reuse matching retained evidence without making upstream calls, so a later explicit refresh does not transiently remove usage, process counts, or their resulting row order. A new definitive orphaned observation replaces retained evidence; timeout or transient unknown evidence leaves the last successful value in place and remains visible through the enrichment counters. Runtime inspection never follows `notLoaded` with `thread/resume`. Neither structural nor enriched snapshots create a Dashboard lease, long poll, control authority, or persistence.
...
Dashboard generation 24 displayed each actual turn's execution selection and added `request`/`manage` row controls. Generation 29 removes `manage` from the current row schema and renderer. Only `controlKind: "request"` is emitted, and only for an actionable non-ordinary approval/input request. **Review requests** expands below the selected Agent, preserves draft answers and input focus across ordinary refreshes, and closes if the Agent leaves the visible page or its request ends. The current card has no work-stop or background-process management panel. The app-private stop contract remains registered only for retained immutable cards.

Dashboard generation 24 adds optional app-private `statusFilter` (`all`, `running`,
`response-required`, `problems`, or `background`) to dashboard reads and native
companion requests. Explicit filters select the shared current-work/run-history
projection; omission retains legacy active/recent/idle pages. The view echoes
`statusFilter` and adds optional `counts.responseRequired` and `counts.problems`.
Counts are computed for the full selected conversation scope before status
filtering and pagination. Response-needed combines input and approval once per
Agent. Problems use the latest unresolved outcome or recovery evidence and
exclude normal response waits. Schema 18 restores every legacy archived Agent
before snapshots are served. A later successful/running
retry leaves older failures only in history, including when full Jobs and archived summaries coexist. Remaining background processes never hide that latest failed outcome. Terminating rows remain visible;
confirmed background processes are independently filterable. Unknown/deferred
runtime evidence is still reported rather than represented as a confirmed zero.

Current work orders response waits and problems first. Recorded idle-Agent turns
join run history with their original execution evidence and expandable older
turns; idle Agents without a recorded turn are omitted. Modern `idleRows` is
empty. Native menu-bar health uses the problem count, with a compatible legacy
fallback, rather than the old attention count that also includes response waits.
The current UI removes aggregate project/conversation/history/idle counters;
those compatible fields remain available to retained clients. Switching a status
filter invalidates previous pages, pending reads and selected controls. Regular
refreshes preserve inline drafts and history disclosure. No retention, thread
ownership, or upstream connection lifecycle changes accompany this projection.

Dashboard generation 26 opts into `problems: {review, kind, offset}`. Its optional
`problems` output contains a separately paginated collection of per-execution
failures and live runtime problems, exact opaque revision targets, review state,
action availability and bounded termination impact. `counts.problems` is the
unreviewed count across the selected scope. Current/terminal pages are empty for
the dedicated Problems filter. Later success does not remove
unreviewed failed executions. Review/undo preserves outcomes; fresh recheck and
explicitly confirmed stop retry are separate actions. Collection revisions are
scoped to the selected review/type query. See [work-history.md](work-history.md)
for atomic batch behavior, proofs, retention and compatibility.

The current Dashboard descriptor requires generation 29 at the content-hashed URI recorded in `ui-manifest.lock.json`. Retained immutable Dashboard resources remain registered according to the lock-file compatibility policy; generation-4–6 offset calls receive their conditional grouping projections. Retained Agent archive/restore requests are parsed only to return `AGENT_ARCHIVE_REMOVED` without mutation.
...
`codex_activity_rehydrate`, which can discover later work without acquiring a
watcher, completion handoff, or controls. The private metadata validator permits
this source/presentation combination only for an empty full-history opening and
still enforces the read-only, non-owning view contract.
...
| `codex_activity` (retained alias only) | Stable Activity-count/open summary | 1,024 | No full feed or watcher/presentation data |
...
| `codex_cancel` (`target.kind: "activity"`), `codex_cancel` | Stable cancellation outcome/error | 768 | Action/outcome/error only |
