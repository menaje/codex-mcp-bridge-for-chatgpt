...
exclude normal response waits and archived Agents. A later successful/running
...
the dedicated Problems filter. Later success and Agent archive do not remove
...
The current Dashboard descriptor requires generation 26 at the content-hashed URI recorded in `ui-manifest.lock.json`. Retained immutable Dashboard resources remain registered according to the lock-file compatibility policy; generation-4–6 offset calls receive their conditional grouping projections.
