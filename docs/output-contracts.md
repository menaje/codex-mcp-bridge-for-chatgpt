...
## Generation 1 private Dashboard envelope and generation 16 UI
...
Generation 3 added `latestTurn` plus older retained `history` entries without Agent, Job, Activity, thread, or execution identifiers. Generations 4 and 5 added conversation and project grouping pages. Generation 7 introduced the fixed `active → recent terminal → idle` order. Generation 12 makes active and recent presentation Activity-first: each row carries an opaque hashed `activityKey`, same-title Activities remain distinct, and matching Agent rows render below their Activity. Idle remains Agent-first because no Activity is currently running. Generation 13 removes initial tool-result hydration: a cold mount keeps the content hidden behind loading, ignores cached tool-result notifications/globals, and first renders only after a fresh `codex_dashboard_snapshot` succeeds. Generation 14 carries the opaque Activity key into retained turns and pages active/recent rows on whole-Activity boundaries. A page limit remains an Agent-row target and may be exceeded by one indivisible Activity. It also invalidates a mounted view before a page re-entry older than one second or a visibility return older than thirty seconds, so a failed fresh load cannot repaint the discarded stale rows. Generation 15 removes the Agent-derived Activity-heading status and renders every Codex turn state only on its owning Agent row. Generation 16 splits structural data from optional runtime/usage enrichment and embeds only card-specific translations. An ordinary in-place refresh failure after one successful snapshot still retains only that mounted iframe's last successful view; an enrichment failure retains the newly rendered structural view. The latest execution badge is shown once when the idle session selection matches the latest turn; only a differing selection is labeled as the next-run setting. Normal current snapshots omit the larger legacy group pages. The server emits them only when an immutable generation-4–6 card sends `conversationOffset` or `projectOffset`, so older resources can still refresh without imposing their payload cost on new cards. Recent and idle pages initially target 20 representative Agents and append later pages through **Show more**. A response whose offset was clamped replaces that bucket instead of merging incompatible pages, and current active/terminal evidence evicts stale in-mount copies. The idle section starts collapsed on each fresh card and retains its disclosure state through ordinary mounted refreshes.

Within an active or recent Activity, the Agent display name heads its nested row; idle remains Agent-headed. Result-bearing Jobs retain full Dashboard turn data while ordinary retention pruning writes only a result-free summary: Job/scope/Activity correlation, Agent/backend columns already held by SQLite, terminal status/update time, optional start time, and exact effective model/effort plus reroute when known. It never retains prompt, result, error, command, or public-event bodies for this purpose. Each current and retained turn may carry the same opaque `activityKey` used for grouping, so same-title Activities remain distinct and the current Activity title is not repeated in its own expanded Agent history. The Dashboard reads at most 10,000 archived summaries, sends at most 12 older turns per visible Agent, and reports the full history count. Pre-generation-7 summaries can therefore have null start/duration and no execution; the UI shows duration unavailable and never substitutes the current session selection as historical fact. An idle or recovery row may separately expose its current tracked-session selection with `isCurrent: true`.

Retained history starts collapsed and expands below the same Agent without exposing an Agent ID or display alias; its in-memory disclosure state survives ordinary snapshot refreshes in the mounted iframe. Session, thread, and compatibility identifiers are never printed as text. **Open in Codex** requires the exact selected App Server thread to match a retained tracked session and yield a UUID-shaped route. That matching session's creation-time visibility bit controls the link; the current preference is a compatibility fallback only when the matching legacy session lacks the bit. Missing, mismatched, hidden, non-App-Server, or other value shapes are omitted. The exact thread UUID takes precedence over the matching session-tree UUID because forked threads can share the latter. OpenAI separately defines `openai/session` as an anonymized correlation value, not a navigable-route guarantee. When a UUID-shaped host value is captured, active and recent groups render one best-effort **Open conversation** link in the Activity context, while an idle row retains the link to its owning orchestration conversation. GPT navigation is independent of the Codex-app visibility preference. Neither route is probed, and previously retained ChatGPT scopes cannot be backfilled without the deliberately unretained raw host value. The Dashboard CSP permits `codex://threads` and `https://chatgpt.com` for `window.openai.openExternal`, while anchors remain the fallback. Active timing shows only start-to-now elapsed duration and omits last-status-update age. A terminal turn shows start-to-terminal duration when known plus time since that exact outcome. Model selection renders as `model · effort`, or `selected → rerouted · effort` when Codex reports a reroute. The fresh-snapshot decoder accepts private metadata as a raw map or inside complete `mcp_tool_result` or `call_tool_result` wrappers, including JSON-encoded nested results.
...
Enrichment selects active/attention/currently paged rows before calling App Server. It uses at most eight workers, a 400-ms per-probe timeout, a 1.2-second overall runtime budget, and a separate 800-ms weekly-usage timeout. A worker whose uncancellable request times out launches no further request. Confirmed stable observations are cached for five seconds using a backend/thread plus Agent/Job-generation stamp, and a successful background-process mutation invalidates its thread entry. Runtime inspection uses `thread/read` and a loaded-thread-only terminal list; `notLoaded` is never followed by `thread/resume`. Timeout and deferred evidence is projected as unknown/skipped. Neither structural nor enriched snapshots create a Dashboard lease, long poll, control authority, or persistence.
...
The current Dashboard descriptor requires generation 16 at the content-hashed URI recorded in `ui-manifest.lock.json`. Retained immutable Dashboard resources remain registered according to the lock-file compatibility policy; generation-4–6 offset calls receive their conditional grouping projections.

The minimum Activity generation for new descriptors is 12. The current immutable resource is generation 20 at the content-hashed URI recorded in `ui-manifest.lock.json`. Every retained immutable Activity URI from generations 7–20 remains registered and refreshes through app-only snapshot/watch paths. Retained HTML assets are not rewritten or deleted when the minimum advances. UI generation 19 normalizes supported host-result wrappers and exposes retryable hydration failures. Generation 20 retains the generation-11 private bootstrap/view envelopes, paints structural data first, and requests bounded enrichment only for a live mounted proof; historical one-shot views remain read-only and do not enrich.

## Card performance and byte budgets

Every Dashboard and Activity view carries `enrichment: { state, runtimeRequests, cacheHits, timeouts, durationMs, usageTimedOut }`. `codex_diagnostics.performance` reports bounded recent samples for structural projection, enrichment, and serialization with count, p50, p95, max, requests, timeouts, and cache hits. It also reports measured self-contained HTML bytes against fixed caps: Dashboard 112 KiB, Activity 144 KiB, and Settings 192 KiB. The existing hydration limits remain runtime-enforced and regression-tested: Dashboard private view 512 KiB, Activity private view 768 KiB, and app-only structured hydration 1 MiB. Sizes are UTF-8 bytes of `JSON.stringify(value)` for data and raw UTF-8 bytes for HTML.
