...
Retained history starts collapsed and expands below the same Agent without exposing an Agent ID or display alias; its in-memory disclosure state survives ordinary snapshot refreshes in the mounted iframe. The session identifier and compatibility alias are never printed. OpenAI defines `openai/session` as an anonymized correlation value, not a navigable-route guarantee. When a UUID-shaped host value is captured, the row renders a best-effort **Open conversation** link beside the project label; reachability is not probed and previously retained scopes cannot be backfilled without the deliberately unretained raw value. The ChatGPT CSP explicitly permits the `https://chatgpt.com` redirect target for `window.openai.openExternal`, while the anchor remains the browser fallback. Active timing is start-to-now plus time since the last Job status update. A terminal turn shows start-to-terminal duration when known plus time since that exact outcome. Model selection renders as `model · effort`, or `selected → rerouted · effort` when Codex reports a reroute. The UI accepts private hydration both as the legacy raw metadata map and inside complete `mcp_tool_result` or `call_tool_result` wrappers, including JSON-encoded nested results.
...
The current Dashboard descriptor requires generation 7 at the content-hashed URI recorded in `ui-manifest.lock.json`. Retained immutable generation-1 through generation-6 resources remain registered as compatibility revisions; generation-4–6 offset calls receive their conditional grouping projections.

The minimum Activity generation for new descriptors is now 12. The current immutable resource is generation 12 at `ui://codex-mcp-bridge/activity/525aa48e0eb7.html`. All 17 retained immutable Activity URIs from generations 7–12 remain registered and refresh through app-only snapshot/watch paths; together with the current resource, 18 Activity resources are registered. Retained HTML assets are not rewritten or deleted when the minimum advances. UI generation 12 continues to validate the generation-11 private bootstrap/view envelopes above.
