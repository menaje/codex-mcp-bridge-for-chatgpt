...
- the 14 model-visible and 6 app-private current descriptors are the only
  discovery inventory.
...
- `codex_task` requires input version 6 and the current descriptor's exact
  `executionEnvelopeRef`;
- task output uses contract version 3 and structured `nextActions`;
...
The release catalog is version 4 and selects one current file each for Settings,
Dashboard, and Decision. Its compatibility arrays are empty. The current resource
...
1. render and synchronize the generated manifest, lock, and three current files;
