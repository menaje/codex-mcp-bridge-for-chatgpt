...
필요하다. 현행 구성은 공개 12개와 카드·제어용 앱 전용 14개, 총 26개다.
이전 `codex_input`은 `codex_status`의 `input` 조회로 통합되었고 더 이상
등록되지 않는다. 새로고침 후에도 이전 리소스 또는 스키마가 보이면 현재 빌드를
배포한 뒤 연결을 새로고침하고 카드를 다시 연다.
...
GPT가 처리한다는 표시만 보여준다. [현재 도구 계약](card-tools.md)을 참고한다.
...
미제출 선택값과 입력은 같은 브라우저 탭의 세션 저장소에 임시로 보존한다.
언어 변경으로 페이지가 다시 로드돼도 현재 질문의 범위·revision·필드와
만료 시각이 일치할 때만 복원하며, 다른 질문과 섞지 않는다. 제출·취소·만료
상태를 확인하면 지운다. 초안은 GPT나 도구 응답에 전달하지 않는다.
호스트가 브라우저 저장소를 막으면 현재 카드가 열린 동안의 초안 보존만
가능하며, 기기 간 동기화나 브라우저 세션 종료 후 보관은 제공하지 않는다.
