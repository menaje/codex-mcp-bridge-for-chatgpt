...
- `codex_agent_recovery_detach` is private, disabled by default, and requires an
  ...
  SQLite transaction.
- `codex_models` returns only the target backend's neutral, policy-allowed model,
  effort, and service-tier support with upstream descriptions plus source and
  freshness. It exposes no recommendation, rank, default, fallback, policy
  summary, or Priority state.
...
- `codex_update_settings` is app-only and exposes one discriminated reset/patch
  ...
  uses generation 14 and never paints cached initial editor metadata; retained
...
- Operator model ceiling ∩ versioned user policy ∩ backend catalog/capability ∩
  ...
  mapping, ranking, recommendation, default, fallback, or effort glossary.
  Missing selection fails with `MODEL_SELECTION_REQUIRED` before any execution
  identity, filesystem preflight, session, or upstream call. Continue/fork
  omission inherits the retained thread while an explicit selection requests an
  allowed backend override. No bridge-maintained model aliases are interpreted.
