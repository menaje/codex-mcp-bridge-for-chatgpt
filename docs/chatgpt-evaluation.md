...
| E9-6 · 언어 전환 | 실제 호스트 언어를 바꾸고 자동 언어 카드의 문구·초안·선택값을 확인한다. 제공되는 미지원 언어에서는 정의된 fallback을 확인한다. | 2026-09-10 실제 영어 전환과 체코어→영어 fallback을 확인했다. 호스트의 전체 페이지 재로드에서 미제출 선택값 손실을 발견해 같은 탭의 질문별 초안 복원을 수정했다. 설치된 첫 수정의 실제 재검증에서도 초안 손실을 확인했다. 브라우저 세션 저장소에 더해 ChatGPT의 비공개 위젯 상태를 사용하도록 보완했고, 저장소가 차단된 재진입 회귀를 추가했다. 이 후속 수정의 실제 ChatGPT 재검증은 남긴다. |
| #68 · 질문 후속 전달 장애 | 현재 독립 질문 카드에서 거절·미지원·불확실한 전달·범위 충돌을 구분한다. 저장 답변 재조회와 명시적 복구가 가능해야 하며 불확실한 쓰기를 자동 재전송하지 않는다. | 2026-09-10 실제 재열기·제출·취소·만료와 범위 경계를 추가 확인했다. B의 정확한 교차 범위 조회 입력을 검사했고 GPT가 `ANSWER_UNAVAILABLE`을 보고했다. 초기 A에서는 명시적 후속 요청 뒤 저장 답변을 읽었다. 이후 Q68-CURRENT-0910에서는 제출 후 추가 요청 없이 GPT가 `codex_user_answer`로 선택값과 메모를 읽었다. 전달 거절·미지원·불확실한 전달의 실제 호스트 장애 주입은 미완료다. |
