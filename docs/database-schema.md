...
| `completion_outbox` | Durable completion follow-up selection, dispatch, acknowledgement, and uncertainty state | Keep delivery authority independently of Dashboard presentation. A verified host conversation event may be selected when available; otherwise an automatically opened origin Dashboard may use bounded `ui/message`. Pending and uncertain records are retained so another channel cannot duplicate an unconfirmed delivery. |
