...
사용자가 잠금을 해제한 뒤 통합 후보를 잠시 적용하고 실제 ChatGPT에서
짧은 설정 설명과 `codex_models` v2의 `automatic`/허용 모델 4개 응답을
확인했다. 독립 질문 카드 제출 뒤 종료된 GPT 응답에 후속 처리가 시작되고,
GPT가 저장된 응답을 읽어 정확히 출력하는 흐름도 성공했다.

이전 도구 정의를 이미 읽은 설정 대화는 연결 새로고침과 페이지 재진입 후에도
긴 설명과 구형 모델·작업 입력을 유지했다. 새 대화 또는 새로 검색한 정의의
성공을 모든 기존 대화의 갱신으로 해석하지 않는다. 별도 실행 검증은
ChatGPT 자동 검토가 작업 생성 전에 거부했고 재시도하지 않았다. 따라서
활성 작업 재시작과 Job 완료 알림·인계 검증은 여전히 미완료다.

기존 작업 476개·Activity 204개·Agent 235개와 설정·프로젝트 테이블을
보존했으며 원래 운영 실행 파일로 복구했다. 상세 범위와 근거는
[잠금 해제 후 검증](2026-09-08-issue-70-unlocked-host.md)에 있다.
