...
When GPT invokes a model-visible cancellation tool, its bounded reason is also
projected into the Activity and Dashboard cards as an initially collapsed
detail. A Job-target reason includes the saved Agent display name so a shared
Activity does not leave the cancelled participant ambiguous. Widget, operator,
and internal containment cancellations remain in the durable audit but are not
presented as GPT-authored explanations.
...
`Keep new Agent tasks in the Codex app` defaults off. With the App Server backend, newly created and forked threads are therefore ephemeral and stay out of the Codex app list unless the user enables this preference; existing threads are unchanged. The native menu-bar overview offers the same change as a one-click, automatically saved action and then shows **Open in Codex** on each eligible Agent row. Ephemeral threads live only in their App Server worker and cannot be resumed after that worker or the bridge restarts. The current MCP Server tool contract has no ephemeral-thread option, so the setting explains that it applies only after switching the bridge backend to App Server.

Opening Settings returns only a compact, path-free public summary. The generation-15 card ignores cached initial tool-result hydration, shows loading, and renders only after the app-private `codex_settings_snapshot` reads the current persisted settings and project registry. That snapshot resolves the model catalog through the normal short-lived cache. There is no persistent refresh control or polling; when the lookup is stale or fails, the card keeps the last-known-good catalog and shows a contextual retry action that refreshes the catalog through the same app-private data path. Its self-contained HTML includes only the Settings, common, and effort translation namespaces.
...
`codex_activity` defaults to `mode: "full-history"` for an explicit user-requested open or reopen. Compact-monitor calls create the automatic presentation class and own the bounded live watch and completion-handoff path; full-history calls create the explicit class, use separate bounded watcher admission, and never claim automatic handoff. Its compact model-visible result now retains the presentation mode plus optional Activity identity/version, allowing a cold-remounted full-history card whose private metadata was discarded by the host to reconstruct one read-only, non-owning full snapshot. Retained pre-decoupling Task resources may still use the same `codex_activity_rehydrate` endpoint for historical mounts, but new Task descriptors cannot create those shells.
...
The automatically mounted card is a compact operational feed for the current ChatGPT conversation. Work that is running, terminating, blocked on the user, awaiting verification/handoff, or needs recovery remains in the active section. Up to three recent terminal or idle Activity rows follow in expanded form, while up to three otherwise-unrepresented idle Agents are available under an initially collapsed section. Rows are ordered by user block, recovery, result review, and progress; ties use most-recent change time and then Activity identity for stability. Each row shows the Activity title, named Agent participants, separate roles, kind, timing, each participant's current or latest effective model/reasoning-effort selection, and only the action needed now. The primary state follows current work, so an active retry remains **Running** instead of being overwritten by an earlier failed Job. A localized previous-failure count appears as secondary context only when that count is positive and the primary state is not already **Failed**. Models use the same catalog display names as Settings and fall back to their internal IDs only when no display name is available. If App Server reports a model reroute, the card shows the admitted model and rerouted model as `selected → rerouted`; the effort remains the admission-time effective effort. It has no KPI dashboard, card-grid Agent list, or layout selector.

The automatic payload stays bounded: recent Activity and distinct idle-Agent detail are capped at three rows each, and an idle Agent already nested in a visible recent Activity is not repeated. One localized summary still reports the exact distinct Activity and Agent totals, for example **Past records · Completed activities 12 · Ended activities 3 · Idle agents 2**. An Activity is folded only after its lifecycle is actually completed and it has no active/terminating Job, pending interaction, verification, completion handoff, open assignment, or App Server background process. Counts are Activity-centric, so reusing an Agent does not erase an earlier completed Activity.

When the user explicitly asks for all work, `codex_activity` opens the full scoped view. It reuses the same row calculation and rendering engine as the compact monitor, but returns current and historical Activity rows plus idle Agents through a bounded opaque cursor. The cursor is tied to the conversation scope version and safely resets when live ordering changes. An exact requested Activity opens on the page that contains it. A cold remount first reconstructs the same paginated full projection as a one-shot snapshot; pressing refresh deliberately promotes it to the ordinary live explicit-card lease. Neither form acquires automatic completion-handoff ownership, and detailed Job answers, events, paths, and diagnostics remain on their existing exact-status or private surfaces.

### Bridge-wide Codex overview

`codex_dashboard` is a separate explicit card for the personal bridge. Ask for the “Codex overview across all conversations” in any connected ChatGPT conversation, then keep that conversation as the overview location. This personal, single-user view is registered unconditionally: there is no Dashboard operator flag or extra per-user configuration. Reopening the conversation, returning to the page, or pressing refresh calls the app-private read-only `codex_dashboard_snapshot`; the card intentionally does not hold a long poll, Activity watcher slot, completion-handoff owner, or control lease. A dormant card cannot receive server-pushed UI updates while the host has not mounted it, so re-entry and manual refresh are the reliable update points. Generation 18 preserves the fixed order **Active Activities → Recent Activity outcomes → Idle Codex agents** and uses the same Activity → Agent hierarchy in every section. Idle Agents are grouped under their latest Activity, explicitly labelled as historical context; Agents without retained work share a **No recent Activity** group within their conversation/project context. Activity headings carry shared project and conversation context once, while each owning Agent keeps its Codex turn state, work time, actual model/reasoning effort, and any distinct next-run setting.

“All conversations” means every conversation scope that this bridge can still identify from live Jobs, result-free archived Job summaries, non-archived Agents, or tracked threads—not the user's complete ChatGPT history. By default, result-bearing Jobs expire after six hours and at most 100 are retained; the Dashboard reads at most 10,000 archived summaries and the tracked-thread registry retains at most 1,000 entries. Archived summaries contain no prompt, result, error, command, or event body. The public call still returns a redacted aggregate text/structured summary for model and non-UI clients, but it no longer stores the full Dashboard view in initial tool-result metadata. Since generation 17, every cold mount first requests an `enrich: false` structural snapshot, paints it without waiting for App Server probes or weekly usage, and then applies an `enrich: true` snapshot in the background. When the ChatGPT compatibility bridge is present, the cold request starts through it immediately instead of waiting up to five seconds for standard MCP Apps initialization. A page re-entry older than one second, or a visibility return older than thirty seconds, clears the mounted snapshot and repeats the same structural-first sequence. Ordinary in-place refresh failures after a successful snapshot still keep that iframe's last successfully loaded view, while an optional enrichment failure leaves the structural view usable.

Enrichment selects active/attention and currently paged Agent rows before any runtime work. It launches at most eight App Server probes concurrently, gives each probe 400 ms, stops a timed-out worker from adding more uncancellable work, and applies a 1.2-second total runtime budget plus an independent 800-ms weekly-usage budget. A five-second cache is keyed by backend/thread and Agent/Job generation; mutations that terminate a background process invalidate the relevant entry. `thread/read` and the loaded-thread-only background-terminal API are read-only: an unloaded historical thread is reported as unknown and is never resumed for display. Timed-out and uninspected rows remain explicit unknown/skipped evidence instead of blocking the card.

Primary status is calculated only from Codex-owned runtime facts: exact Job running/terminal/termination state, tracking liveness, Agent lifecycle, pending input or approval interactions emitted by Codex, and confirmed App Server background-terminal counts. It never reads Activity `lifecycle`, `waitingOn`, `verification`, completion handoff, or GPT's judgment that a user goal is done. Consequently **Codex turn completed** means only that the retained Job status is exactly `completed`; `failed`, `interrupted`, and `cancelled` remain separate outcomes. The **Attention states** summary is deduplicated per Agent from its latest retained outcome, so an older failed attempt no longer keeps the count elevated after a later running or completed retry. It is a neutral count of Codex states that warrant notice, not an instruction that the user or GPT must verify them. A remaining App Server terminal is shown separately as **Background process running** and is not treated as a running Codex turn. **Tracked projects** counts active, non-deleted project registrations, including an active registration whose folder is temporarily unavailable. Archived and deleted registrations and the unassigned compatibility bucket are excluded, while their retained Activity and Agent rows remain visible as history.

Generation 7 renders representative Agent rows directly in their Codex status buckets and omits project/conversation group pages from normal snapshots. The server computes those larger projections only when an immutable older card sends its compatibility offsets. Generation 14 returns active and recent pages on Activity boundaries: the nominal limit is still measured in Agent rows, but one Activity is never split, so a page may exceed the limit when a single Activity has many Agents. Generation 15 removes the Agent-derived status badge from each Activity heading so `Codex turn completed` and other turn states appear only on Agent rows. Generation 16 adds structural-first progressive hydration and card-specific translation payloads. Generation 17 extends Activity-boundary paging and Activity-first rendering to idle Agents. Generation 18 keeps actual and next-run model/effort details on each owning Agent instead of hoisting identical values, omits the enclosing Activity title from same-Activity history entries, and emits a neutral **Previous Activity** boundary when distinct same-title Activities meet. Recent and idle sections use **Show more** to append in place instead of replacing the visible list with previous/next pages. An opaque stable row key prevents a renamed Agent or moved status from leaving a duplicate cached row, and a clamped offset rebases the affected page. The idle section starts collapsed on every fresh card and preserves its disclosure state during ordinary refreshes in the mounted iframe. Each Agent appears once in its current bucket, while up to 12 older calls are sent inside that Agent's collapsed history with the full retained-history count shown.

Rows expose only stable hashed conversation, project, Activity, and internal row correlation keys, a compatibility conversation alias in the private contract, the user-defined project label, display Agent name, optional Activity title, Codex status, timing, a background-process count, effective model/reasoning-effort selection, and narrowly validated private navigation targets. Project UUIDs and paths are never projected. Every section is Activity-headed with Agents nested below; idle groups describe the latest retained Activity rather than a current assignment. Older retained calls are grouped newest-first inside an initially collapsed Agent history. The same opaque Activity key is carried into each turn, so two same-title Activities remain distinct and the current Activity title is not repeated inside its own history entry. Every visible actual turn keeps its own model/effort line; turns predating execution retention say **Model · reasoning unavailable**, while the current tracked-session selection is labeled separately as a next-run setting and is never substituted for unknown history. Archived rows created before duration retention show **Work time unavailable** instead of a fabricated zero. The card never prints a session ID, thread ID, or alias as text. **Open in Codex** is emitted only when the exact selected App Server thread has a matching retained tracked session, a UUID-shaped route, and an explicitly retained creation-time visibility bit set to true. Legacy sessions without that provenance are omitted rather than inferred from the current preference. A missing, mismatched, hidden, non-App-Server, or non-UUID session is omitted. The exact current thread UUID is preferred because forked threads can retain a shared session-tree ID; the matching session UUID is only a fallback. The route is not probed. The native macOS app additionally checks that macOS has an application registered for the `codex:` route before showing its button. ChatGPT on iOS can access Codex tasks through a paired Remote host, but `codex://threads/<uuid>` is not treated as a documented cross-device route; the host may open it only when it can resolve that local task and connection. Separately, OpenAI defines `openai/session` as an anonymized correlation value rather than a navigable-route guarantee. When the host supplies a UUID-shaped value, the bridge retains a bounded private scope-to-value mapping and renders a best-effort **Open conversation** link to `https://chatgpt.com/c/<uuid>` once in the Activity context; this GPT-orchestration link is independent of Codex-app visibility. Idle groups retain the same GPT navigation for their owning conversation. Arbitrary host session values are neither persisted nor linked, and scopes retained before this capture cannot be backfilled because their raw values were intentionally not stored. The Dashboard CSP allows both validated navigation targets for `openExternal`, and the ordinary anchor remains the fallback. Active rows show **Work time** from Job start to now and omit status-update age. Completed, failed, interrupted, cancelled, and idle retained turns show **Work time** when known plus relative time since that exact outcome; absolute start and end timestamps are not rendered. Model catalog display names are preferred, the model ID is the fallback, and a reported runtime model reroute is shown as `selected → rerouted` while effort remains the admission-time effective effort. If an ordinary in-place refresh transport fails after the first fresh snapshot, the card keeps that mounted iframe's last loaded snapshot, reports the stale state non-destructively, suppresses repeated automatic retries, and lets the refresh button retry explicitly. A standard call that was already dispatched is not duplicated through the compatibility alias after its timeout. Outside those private navigation targets, raw Job, Activity, Agent, thread, worker, and process IDs; filesystem paths; prompts; results; errors; commands; and diagnostics are absent. Display labels, Activity titles, and the navigation links can still reveal or navigate to cross-conversation task context, which is why this card belongs only on the bridge's single trusted user's connection. The widget-instance UUID is mount correlation, not authentication. The card has no cancel, steer, approval, handoff, archive, or process controls.
