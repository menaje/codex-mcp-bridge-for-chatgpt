...
const prelude=`<script>(()=>{
 const fixture=${JSON.stringify(fixture)};let sequence=0,ack=false,archived=false,cleaned=false;
 ...
 const epoch=Date.now();window.openai={locale:'ko',notifyIntrinsicHeight:()=>{},callTool:async(name,args)=>{
  ...
  if(name==='codex_ui_history'){if(args.token!=='fixture-proof'||!args.requestId)throw new Error('Missing history proof');if(args.action==='acknowledge')ack=true;if(args.action==='archive')archived=true;if(args.action==='restore')archived=false;return {structuredContent:{ok:true}}}
  ...
  const ended=new Date(epoch-7200000).toISOString(),base=fixture.terminalRows[0],row={...base,rowKey:'a'.repeat(32),agentName:'기록 검토 에이전트',activityTitle:'이전 작업 확인',status:'failed',updatedAt:ended,
   ...
   historyControls:{revision:'b'.repeat(64),canAcknowledge:!ack&&!archived,canArchive:!archived,canRestore:archived,archived}};
  ...
  const rows=cleaned||args.statusFilter==='running'||args.statusFilter==='response-required'||args.statusFilter==='problems'&&(ack||archived)?[]:[row];
  const activeRows=args.statusFilter==='problems'||args.statusFilter==='response-required'?[]:[running];
  const result={...fixture,statusFilter:args.statusFilter||'all',generatedAt:new Date(epoch+ ++sequence).toISOString(),filter:{mode:'all',conversationAvailable:false,conversationHasWork:false},activeRows,terminalRows:rows,
   counts:{...fixture.counts,running:1,responseRequired:0,problems:ack||archived||cleaned?0:1},historyPolicy:{retentionDays:30,issueAttentionDays:7,lastCleanupAt:ended,lastCleanupCount:12,totalRemoved:cleaned?13:12},
   pagination:{...fixture.pagination,active:{...fixture.pagination.active,returned:activeRows.length,total:activeRows.length},terminal:{...fixture.pagination.terminal,returned:rows.length,total:rows.length}}};
...
try{
 ...
 await cli("run-code",`async page=>{
  ...
  check((await page.locator('#active-list .time').innerText()).includes('시작'),'Current work has relative start time');
 ...
 await cli("run-code",`async page=>{
  ...
  await page.getByRole('button',{name:'에이전트 보관',exact:true}).click();await page.getByRole('button',{name:'에이전트 복원',exact:true}).waitFor();
  await page.getByRole('button',{name:'에이전트 복원',exact:true}).click();await page.getByRole('button',{name:'에이전트 보관',exact:true}).waitFor();
  ...
  if(calls.join(',')!=='acknowledge,archive,restore')throw new Error('Wrong or repeated mutations');
 }`);
 const report={passed:12,artifacts,checks:["current/failed split","live relative time","unknown duration","expanded history time","policy notice","cleanup count","360/460 widths","dark mode","acknowledge","archive/restore","single mutation dispatch","cleanup refresh"]};
