...
const renderer = await import((builtRenderer ? "../dist/" : "../src/") + "questionCard.js");
const CARD_HTML = renderer.QUESTION_CARD_HTML;
...
const connection = await connectCurrentMcpServer(server, { name: "question-browser", version: "1" });
...
for (const mode of ["standard", "metadata-missing", "denied", "uncertain", "expired", "read-retry", "storage-denied", "host-private-state"]) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      if (url.pathname === "/call") {
        ...
        if (mode === "expired" && input.name === "codex_ui_read" && result._meta?.[USER_QUESTION_META]) result._meta[USER_QUESTION_META].expiresAt = Date.now() - 1;
    ...
    response.end(`<html><head><meta charset="utf-8"></head><body style="margin:0"><iframe id="card" style="border:0;width:100%;height:650px" src="/card?mode=${mode}"></iframe><script>
      ...
      window.addEventListener('message',async event=>{if(event.source!==frame.contentWindow)return;const value=event.data;if(!value||value.jsonrpc!=='2.0')return;
        ...
        if(value.method==='ui/initialize'){reply({protocolVersion:'2026-07-28',hostContext:{locale:'ko-KR'}});return}
...
try {
  ...
  for (const [mode, record] of records) {
    ...
    if (mode === "expired") {
      ...
      if (mode === "standard") {
        ...
        await cli("run-code", `async page=>{const frame=page.frames().find(f=>f.url().includes('/card'));if(await frame.evaluate(()=>Object.keys(sessionStorage).some(key=>key.startsWith('codex.question.draft.v1:'))))throw new Error('Submitted draft remains in browser storage')}`);
      }
    }
    ...
    assert.equal(record.calls.filter(name => name === "codex_question_action:submit").length, mode === "expired" ? 0 : 1);
  ...
  await writeFile(path.join(artifacts, "results.json"), JSON.stringify({ builtRenderer, protocolVersion: "2026-07-28", codexCalls, results }, null, 2));
  ...
  await connection.close(); await server.close(); store.close(); await rm(root, { recursive: true, force: true });
}
