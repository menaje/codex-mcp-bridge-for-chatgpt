            ...
            if model.needsSetup {
                ...
                TabView {
                    ...
                    ConnectionSettingsPane()
                        .environmentObject(model)
                        .tabItem { Label("연결", systemImage: "point.3.connected.trianglepath.dotted") }
                    DiagnosticsPane()
                        .environmentObject(model)
...
private struct ConnectionSettingsPane: View {
    ...
    @State private var showForceRestart = false
    ...
    var body: some View {
        Form {
            Section("런타임") {
                LabeledContent("상태", value: phaseLabel(model.helperStatus?.phase))
                LabeledContent("브리지 연결", value: model.helperStatus?.bridge.connected == true ? "연결됨" : "연결 안 됨")
                LabeledContent(
                    "활성 작업",
                ...
                HStack {
                    ...
                    Button("작업을 마치고 재시작") { Task { await model.restartRuntime(force: false) } }
                    Button("강제 재시작…", role: .destructive) { showForceRestart = true }
                    Spacer()
                    Button("작업을 마치고 중지") { Task { await model.stopRuntime(force: false) } }
                    Button("강제 중지…", role: .destructive) { showForceStop = true }
                }
                .disabled(model.isBusy)
                Button("Tunnel 프로필 복구…") { showRepairConfirmation = true }
                    .disabled(model.isBusy || model.helperStatus?.configuration.valid != true)
            }

            Section("Private dotenv") {
                LabeledContent("파일", value: model.helperStatus?.configuration.path ?? "")
                    .textSelection(.enabled)
                LabeledContent("Runtime API key", value: model.helperStatus?.configuration.hasApiKey == true ? "저장됨" : "없음")
                HStack {
                    LabeledContent(
                        "Tunnel ID",
                        value: model.helperStatus?.configuration.tunnelId ?? "없음"
                    )
                    if let tunnelId = model.helperStatus?.configuration.tunnelId {
                        Button {
                            NSPasteboard.general.clearContents()
                            NSPasteboard.general.setString(tunnelId, forType: .string)
                        } label: {
                ...
                SecureField("새 키를 입력할 때만 교체", text: $apiKey)
                TextField("새 Tunnel ID를 입력할 때만 교체", text: $tunnelId)
                Button("연결 정보 저장") {
                    Task {
                ...
                .disabled(apiKey.isEmpty && tunnelId.isEmpty || model.isBusy)
                Text("Keychain을 사용하지 않습니다. 빈 값은 기존 값을 유지하고, 앱은 저장된 API key 전체를 다시 표시하지 않습니다.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Section("Codex 로그인") {
                Label(
                    model.authStatus?.summary ?? "상태 확인 필요",
                    systemImage: model.authStatus?.authenticated == true
                        ? "checkmark.circle.fill"
                        : "person.crop.circle.badge.exclamationmark"
                )
                Button("브라우저 로그인 시작") { Task { await model.launchCodexLogin() } }
            }

            Section("ChatGPT 연결") {
                Text("ChatGPT 설정에서 Developer mode를 켠 뒤 Secure MCP Tunnel을 선택하고 No Auth로 연결합니다.")
                Link("ChatGPT Connector 설정 열기", destination: URL(string: "https://chatgpt.com/#settings/Connectors")!)
            }
        }
        .formStyle(.grouped)
        .confirmationDialog("강제 재시작할까요?", isPresented: $showForceRestart) {
            Button("강제 재시작", role: .destructive) { Task { await model.restartRuntime(force: true) } }
        } message: {
            Text("활성 작업 \(activeWorkCount)개가 중단될 수 있으며 자동으로 재실행되지 않습니다.")
        }
        .confirmationDialog("강제 중지할까요?", isPresented: $showForceStop) {
            Button("강제 중지", role: .destructive) { Task { await model.stopRuntime(force: true) } }
        } message: {
            Text("활성 작업 \(activeWorkCount)개가 중단될 수 있으며 파일 변경은 되돌아가지 않습니다.")
        }
        .confirmationDialog("Secure MCP Tunnel 프로필을 다시 만들까요?", isPresented: $showRepairConfirmation) {
    ...
    private var activeWorkCount: Int {
        (model.helperStatus?.bridge.activeJobs ?? 0) +
            (model.helperStatus?.bridge.pendingAdmissions ?? 0)
    }
}
...
private struct DiagnosticsPane: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("제한된 진단 로그").font(.title2.bold())
                Spacer()
                Button("새로고침") { Task { await model.refreshLogs() } }
            }
            Text("키 형태의 값은 helper에서 마스킹되며 요청 본문과 .env 내용은 로그에 기록하지 않습니다.")
                .font(.caption)
                .foregroundStyle(.secondary)
            List(model.logs) { entry in
                VStack(alignment: .leading, spacing: 3) {
                    HStack {
                        Text(entry.source).font(.caption.bold())
                        Text(DisplayFormat.dateTime(entry.at)).font(.caption).foregroundStyle(.secondary)
                    }
