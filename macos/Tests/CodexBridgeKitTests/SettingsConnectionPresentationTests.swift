...
final class SettingsConnectionPresentationTests: XCTestCase {
    ...
    func testProductionSettingsUsesTheSkillLibraryStyleSidebarAndSeparateDraftDestinations() throws {
        ...
        XCTAssertTrue(source.contains(".navigationSplitViewColumnWidth(min: 210, ideal: 230, max: 270)"))
        XCTAssertTrue(source.contains(".navigationSplitViewStyle(.balanced)"))
        XCTAssertTrue(source.contains("if visibility != .all { columnVisibility = .all }"))
        ...
        XCTAssertFalse(source.contains("SettingsSidebarSearchField"))
        XCTAssertFalse(source.contains("NSSearchField"))
        XCTAssertFalse(source.contains("settings-search-field"))
